...
- A one-line `grep "import\|from"` for importers MISSES multi-line imports — `apps/web/scripts/_embed-backfill.ts` imported the one exported `vectorToPgString` copy inside a `{ … }` block and surfaced only as a typecheck error after the "unused" export was deleted (#1132). Grep the bare symbol name, not the import keyword, before deleting an export. Same session: a plan claimed a warn was "visible in Axiom" when the call site's `logger` is the console-only `@askarthur/utils/logger` — check WHICH logger a file imports before claiming observability. [src: session_01Vw9bTA9ehtAZSBG4kFUiv9]
...
- A worklist's ORDER BY decides whether a full-looking batch is actually starving. `list_clone_alerts_pending_urlscan_submit` is `ORDER BY first_seen_at DESC`, so the 28-of-30 "already-400-rejected" rows I found were padding the TAIL, not displacing fresh candidates — measured throughput was ~80% of gate-passing candidates scanned per ingest day. Two sampling rules from this: (a) never read a cron's worklist composition between the upstream ingest (08:30) and the consumer (09:00) — that's the trough; (b) check the ORDER BY before concluding that queue composition implies starvation. Related: the ~20% of each clone-watch cohort never scanned is mostly domains urlscan REFUSES for having no DNS, not capacity starvation — more slots (or a liveness pre-gate) would not recover them. [src: session_01CaT4vAimGXyiHAAqA4Wsaj]
