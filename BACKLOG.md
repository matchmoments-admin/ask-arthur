...
- [ ] **Full C2PA cryptographic validation** — image-check v2 ships PRESENCE detection only (`packages/scam-engine/src/c2pa-detect.ts`, structural container sniff; copy says "issuer unverified"). Upgrade path: `c2pa-node` (Content Authenticity Initiative, Rust native bindings — verify Vercel Node-runtime build) to validate the manifest signature + issuer chain, turning the card line into "Signed by Adobe Firefly" / "Manifest invalid". Trigger: image-check volume proves demand, or an LE partner asks for validated provenance
