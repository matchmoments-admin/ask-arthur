...
- [ ] **Finish timeouts missing on five apps/web Inngest functions** (recorded 2026-09-10, #1135) — the seven-function figure counted scam-engine only; the real fleet gap was twelve. Still uncovered: `clone-watch-notify-brand`, `clone-watch-submit-netcraft`, `clone-watch-weekly-digest`, `phone-footprint-pdf`, `phone-footprint-vonage-backfill` (the last two belong to a mothballed feature — see NORTH_STAR.md). They are now held by an asserted allowlist in `apps/web/__tests__/inngestFinishBudgets.test.ts`, so no NEW function can ship without a finish timeout and an entry that gains one must leave the list. Each needs its own boundary derivation: `boundaries x 30s + inline wall-clocks + 60s slack`
