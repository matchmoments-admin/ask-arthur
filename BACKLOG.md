...
- [ ] **Budget the `kit-pivots` step** in `clone-watch-enrich-attribution` (recorded 2026-09-09, #1131) — one urlscan search per row, sequential, quota-bounded, now reporting a Write Outcome but still no Step Budget. Convert to `budgetedStep` with a `KIT_PIVOT_WALL_CLOCK_MS` literal and re-derive that function's finish floor (`inngestFinishBudgets.test.ts` will demand it). Second real adapter of the Module; also the moment to revisit the deferred worklist-drain seam (a drain needs both a Step Budget and a Write Outcome)
