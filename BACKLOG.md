...
27. - [ ] **Clone-watch v2 matcher — context-token gate to cut common-English-word FPs.** First prod run (2026-05-24) after `MIN_BRAND_LEN_FOR_LOOSE_SUBSTRING = 5` defence still had ~70% FP rate on common-word brands: 3 hits on "Reece" were all _Greece_ (`bigclash-greece.co`, `greeceexcursion.com`, `spindjinn-greece.net`); 7 hits on "Target" mostly real businesses (`targetsec.com.br`, `targettcarpentryltd.co.uk`, `pricelineevsunbury.com` for Priceline). Approach options: (a) require a scam-context token within N chars of the brand (`bank`, `login`, `support`, `ads`, `online`, `secure`, `verify`, `pay`, `home`, etc.), (b) Voyage embedding distance from brand-name vs candidate label, (c) per-brand exclusion list (skip if primary label contains `greece`, `coles-creek`, etc.). Defer until ≥7 days of evidence shows the FP rate matters for outreach.
