...
- [x] **Embed every analyze submission → surface "N similar reports"** — SHIPPED (stale entry corrected 2026-08-21): `scam_reports.embedding VECTOR(1024)` + partial HNSW landed in v89, the `scam-report.stored.v1` consumer is `packages/scam-engine/src/inngest/scam-report-embed.ts`, and the consumer surface is `/api/analyze/similar` + `components/result/SimilarReports.tsx` behind `NEXT_PUBLIC_FF_SIMILAR_REPORTS`. The REMAINING gap in this space: submission-campaign clustering — `scam_clusters.cluster_type='text_similarity'` has been a declared-but-never-written enum value since v22 (the daily cluster-builder is entity-graph union-find only); an embedding-based campaign clusterer over user submissions is a real future feature needing its own plan
...
- [ ] **`PRICING.HIVE_AI_USD_PER_IMAGE` constant update** — once the contract is signed, adjust the constant in `apps/web/lib/cost-telemetry.ts` if the negotiated rate ≠ $0.003 (the inline-`0` placeholder was replaced in #782)
- [ ] **Soft-404: `notFound()` on flag-gated pages returns HTTP 200** (found 2026-07-17 while local-testing image-check v2, against a running dev server) — a genuinely nonexistent route correctly 404s, but any page that calls `notFound()` after an `await` returns **200** with the not-found body rendered, because streaming has already flushed the status. Confirmed app-wide and pre-existing: `/charity-check`, `/scam-feed`, `/extension/link`, `/image-check/[ref]`. **No security impact** (the body is identical in every not-found case, so nothing leaks about which refs/features exist) and no impact on `noindex` pages, but `/charity-check` and `/scam-feed` are public indexable consumer surfaces — Google treats a 200 "not found" as a soft-404. Fix options: gate the flag check in middleware (status set before streaming), or hoist `notFound()` above the first `await`. Route handlers are unaffected (they set status explicitly)
- [x] **Full C2PA cryptographic validation** — SHIPPED 2026-08-20 behind `FF_IMAGE_CHECK_C2PA_VALIDATE` (default OFF): `packages/scam-engine/src/c2pa-verify.ts` (`@contentauth/c2pa-node`, dynamic import, runs only after the `c2pa-detect` presence sniff) upgrades the card line to "Content Credentials verified — signed by {tool}" / "manifest invalid — may have been altered". Ops + local-dev binary gotcha: `docs/ops/extension-image-check-config.md`. Remaining follow-ups: (a) **Tier-2 vendor detection APIs** (Anthropic Claude text detection, SynthID Detector, OpenAI verify, ElevenLabs audio — each behind its own flag with `logCost()`): revisit when ALL four trigger conditions hold — callable endpoint (not "forthcoming") AND published FPR ≤1% with a stated minimum reliable input length AND ToS permitting a third-party commercial product to submit end-user text and display results AND viable pricing/rate limits. Reassess quarterly; resale-ToS is the expected blocker (a public detection endpoint doubles as an evasion oracle); the EU AI Act Art. 50 interoperability mandate (2 Feb 2027) is the structural forcing function. (b) NEVER build homegrown statistical text-watermark detection — not because presence-detection is impossible (it isn't — ICLR 2025 black-box detection; ICML 2024 ~$50 stealing) but because without the vendor key it is unattributable, low-precision, and paraphrase-fragile, so it cannot meet the honest-verdict bar; and a genuine positive still only means "processed by, not authored by" (ADR-0024 Amendment 2026-08-21). Deterministic character inspection (invisible-Unicode/bidi scanning in the injection detector) is outside this prohibition
- [ ] **C2PA-for-text + document AI-metadata reader** — C2PA 2.3/2.4 defines provenance manifests for unstructured text, and Office documents carry deterministic AI-tool traces (DOCX `docProps/app.xml` authoring application; Microsoft's "Intelligence data from AI assisted features" property when Copilot contributed). Both are honest, deterministic reads that fit the AI Origin ladder as `claimed`-tier signals ("AI tool metadata present — strippable, not conclusive"). **Trigger: a document-upload surface exists** — today `/api/analyze` takes text + base64 images only and `/api/media` is audio-only, so there is nothing to attach this to; building the upload surface first is its own plan (new MIME allowlist, parser dependency, zip-bomb/XXE threat model)
- [ ] **Style-classifier corroborator (e.g. Pangram)** — if ever surfaced, ONE low-weight corroborating line, never a verdict or accusation (ADR-0024 asymmetry rules apply). Conditions before building: English long-form only (>300 words, well above Pangram's 50-word floor), "corroborating, not conclusive" copy, and independent NON-vendor FPR testing on short/edited text. Current blocker for an AU multilingual user base: vendor-published per-language skew (Urdu 5.3% / Persian 3.2% missed-AI rates vs 0.34% English) fails the honesty bar
...
- [x] **Finish timeouts missing on seven scam-engine Inngest functions** — **CLOSED 2026-09-10** (#1134 + #1135). All seven now declare `timeouts.finish` with a derivation comment. Three needed a real fix first, because a circuit breaker over an unbounded loop is decorative: `embed()` had no request timeout at all, `reddit-intel-daily`'s 360s Claude timeout could never fire under a 300s request (and its output ceiling and request budget were mutually unsatisfiable), and `match-b2b-exposure` wrote an unbounded per-row loop inside one step. The two backfill embedders got spanning budgets rather than reduced batch caps — both loops already exit on a short batch, so the high boundary counts are backfill-only and a cap cut would have traded real throughput for no steady-state gain
- [x] **Finish timeouts missing on five apps/web Inngest functions** — **CLOSED 2026-09-10** (#1139). Every registered Inngest function in both directories now declares one; the allowlist is empty and the guard is per FUNCTION rather than per file. That change surfaced **two more** that the per-file version structurally could not see (`phone-footprint-refresh-monitor` and `enrich-vulnerability-au-context`, each sitting behind a covered sibling in the same file) — the real gap was 7 functions, not 5, on top of the 7 closed in #1134/#1135. It also found a boundary declaration attributed to the wrong function: `enrich-vulnerability.ts`'s `56 boundaries` derives from the cron's `BATCH_SIZE` but sat above the event function, which only mattered once the check became per function
- [ ] **The finish-budget floor double-counts a spanning budget's queue wait** (recorded 2026-09-10, #1135) — `inngestFinishBudgets.test.ts` sums every `*_WALL_CLOCK_MS` declaration and adds `boundaries x 30s` on top. That is right for an **in-step** budget (the queue wait happens outside it) and wrong for a **spanning** one, which measures from `event.ts` and therefore already contains the queue wait. On `scam-reports-backfill-embed` the naive sum gives a 62-minute "circuit breaker" for a run its 600s budget already bounds at ~11 minutes. #1135 worked around it by declaring the boundary count the budget can actually admit (`budget / 30s`) and explaining the derivation in each file, which is honest but subtle. The real fix is for the floor formula to know which kind of budget it is looking at — `StepBudget.kind` already carries that at runtime, but the test reads source. Cheapest option: a distinct suffix (`*_SPANNING_WALL_CLOCK_MS`) the test can treat as `max(budget, boundaries x 30)` instead of a sum
- [ ] **The daily classifier's output ceiling is capped by the request, not by the model** (recorded 2026-09-10, #1134) — `CLASSIFY_MAX_TOKENS` cannot exceed ~14,000 while the call is a single non-streaming `step.run`: at the slowest observed Sonnet emission rate (50 tok/s) a full-length response needs `maxTokens/50` seconds, and the Inngest route's `maxDuration = 300` kills the request first. The two bounds were mutually unsatisfiable at 16,000 and the 360s timeout chosen to honour the first could never fire; #1134 lowered both and added `reddit-intel-output-budget.test.ts` -> "keeps the output ceiling and the request budget mutually satisfiable" so it cannot recur silently. To go **above** 14,000 the shape has to change, not the number: either shrink the output per call (`BATCH_SIZE` is 40 in `apps/web/app/api/cron/reddit-intel-trigger/route.ts` — halving it halves the tokens but doubles the runs, which cuts against the Inngest step-run budget), or switch to `.stream()` + `getFinalMessage()` (what the Anthropic SDK wants past ~16K anyway), or raise the route's `maxDuration` (fleet-wide — it is the ceiling every in-step budget derives from, so it needs its own ADR note). Trigger: the `reddit-intel-truncated` marker rate rising again
- [x] **Budget the `kit-pivots` step** — **CLOSED 2026-09-10** (#1136). `KIT_PIVOT_WALL_CLOCK_MS` (120s) via `budgetedStep`, checked per row. The step's finish floor rose past the declared 33m — which had been sitting EXACTLY on its floor with zero headroom — so `finish` went to 35m; the guard going red was the mechanism working, and the go-red for it is recorded in the PR. Also closed the #1131 gap: `notReachedQuota` was computed and logged but not returned, so a caller saw up to a nine-row hole with no field explaining it. The loop's decision now lives in `runKitPivots` (`apps/web/lib/clone-watch/kit-pivot.ts`) so all four outcomes — written, failed, abandoned to quota, never reached — are tested by calling it rather than grepping it
- [x] **Fold the enricher's 60-step per-item fan-out into one batched step** — **CLOSED 2026-09-26** (#1229 part 1, v331). One `enrich-batch` budgetedStep (200s, 4 wide, starts ≥3s apart so whoisjson stays ≤20/min), an `attribution IS NULL` read-back at the start of every attempt, and chunked bulk writes through `apply_clone_alert_attributions` (10 rows/chunk, fills only `attribution IS NULL`). 64 declared boundaries → 5, finish 36m → 10m. Logic in `apps/web/lib/clone-watch/enrich-attribution-batch.ts`, tested by calling it (`__tests__/enrichAttributionBatch.test.ts`). Original note follows. (recorded 2026-09-10, #1136) — `clone-watch-enrich-attribution` spends 60 of its 64 step boundaries on `enrich-${alert.id}`, one `step.run` per row up to `ENRICH_RUN_CAP`. Folding takes it to ~5 boundaries and its finish from 35m to ~8m: **the single largest step-run reduction available in the fleet**, and `docs/ops/inngest-slot-budget.md` already nominates it. NOT a free win, which is why it is still open: per-item steps checkpoint completed rows, so a folded batch re-runs up to 60 rows of PAID lookups (whois / RDAP / CT / AbuseIPDB) on a mid-batch retry under `retries: 2`. Do it with an idempotency read-back at batch start — the `persistAssignments` pattern (`#520` H5) — so a retry skips rows already enriched, and the fold stops trading provider spend for slot-seconds
...
- [x] **v0.2e — main-checker auto-detection deep-link** — DONE 2026-08-07 (charity-check/v02e-inline). Detection had shipped earlier (`packages/scam-engine/src/charity-intent.ts`, checksum-validated ABN, keyword precision gate); this change completed the spec: CTA now runs the register check INLINE on the homepage (no page hop — reuses ScamChecker's inline charity flow), `charityIntent` rides the `analyze.completed.v1` event (shopSignal precedent) and survives cache hits, and the ABN-only deep-link dead-end (Name tab, invisible prefill) is fixed with `mode=abn`
