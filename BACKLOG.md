...
- [ ] **Finish timeouts missing on seven scam-engine Inngest functions** (recorded 2026-09-09, #1130) — ADR-0019 calls `timeouts.finish` "this ADR's circuit breaker", and `inngestFinishBudgets.test.ts` _skips_ any function without one, so these are unbounded across step boundaries and invisible to the floor check: `acnc-charity-backfill-embed`, `analyze-failure`, `match-b2b-exposure`, `on-demand-url-enrich`, `reddit-intel-daily`, `reddit-intel-embed`, `scam-reports-backfill-embed`. `reddit-intel-cluster` got its own arithmetic in #1130 (7 boundaries × 30 s + 240 s + 240 s inline + 60 s slack = 750 s → 13m). Each of the seven needs the same derivation — boundaries counted (declare `inngest-finish-budget: N` where a loop makes static counting wrong), inline `*_WALL_CLOCK_MS` budgets via `budgetedStep`, then the floor — not a guessed number: a finish tuned for fast dispatch CANCELS healthy runs silently (#1069, 44 of 51 preclassify runs)
