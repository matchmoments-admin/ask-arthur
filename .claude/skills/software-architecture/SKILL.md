...
- **Defect-shape guards**: ~17 Vitest fitness functions asserting a property
  over source or configuration rather than a single case — `rowCap.test.ts` (the
  archetype), `adminErrorBand`, `featureGateRuntime`, `inngestFinishBudgets`,
  `inngestBrakesMatrixDrift`, and others. **Indexed with their defect classes and
  the gaps in [`docs/agents/defect-shapes.md`](../../../docs/agents/defect-shapes.md)** —
  read it before adding a rule, so a new guard extends the set instead of
  duplicating one.
