...
| **User-scan reports** (F1)       | n/a — public-facing alias                                                                     | `scan+report@askarthur-inbound.com`<br/>**direct path is the only one currently working — `scan@askarthur.au` M365 leg silently drops** (see data-flows §5b "M365 dead-leg"). Currently pilot-only, shared privately with friends. | `inbound_scan`         | **NOT a feed_items source** — routed to `/api/inbound-scan` instead. See data-flows §5b. |
