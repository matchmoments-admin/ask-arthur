...
| **User-scan reports** (F1)       | n/a — public-facing alias                                                                     | `scan+report@askarthur-inbound.com`<br/>(alias: `scan@askarthur.au`) | `inbound_scan`         | **Live in prod 2026-05-17.** NOT a feed_items source — routed to `/api/inbound-scan` instead. See data-flows §5b. Cloudflare rule id `b01bd80e63524b57b5eb40130f96dde7`. M365 side uses an **Exchange transport rule** (not inbox rule — alias resolution rewrites `To:` before inbox rules evaluate) + "Automatic forwarding: On" in security.microsoft.com anti-spam outbound policy. Template upgrade tracked in #281. |
