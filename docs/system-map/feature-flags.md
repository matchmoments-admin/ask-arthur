...
| `rdapLookup` (`FF_RDAP_LOOKUP`)                                                   | OFF     | Registrant-intel (2026-07-17). RDAP-first domain-registration lookup in clone-watch attribution (`lookupDomainRegistration`), whoisjson fallback only when RDAP is empty. Adds EPP `statuses` (clientHold/serverHold = registrar-suspended), registrar IANA id, structured abuse contact. Server-side only; free/unmetered. OFF ⇒ whoisjson-only (byte-identical to before).                                                                                                                    |
| `cloneWatchAuRegistrant` (`FF_CLONE_WATCH_AU_REGISTRANT`)                         | OFF     | Registrant-intel (2026-07-17). For `.au` clone alerts, fetch the auDA-disclosed registrant legal name + ABN (RDAP) and cross-check the ABN against the ABR register; cancelled/not-found/mismatched feeds `weaponisation-risk`. Stored in `attribution.au_registrant`. **Independent of `rdapLookup`** (own RDAP fetch). Legal name stored only for non-individual entities (PII guard, v236). Server-side only; free.                                                                          |
| `cloneCampaigns` (`FF_CLONE_CAMPAIGNS`)                                           | OFF     | Registrant-intel (2026-07-17). Stamps `shopfront_clone_alerts.campaign_key` (v235; sha256 of registrar + NS roots + ASN + cert issuer) so the report card + `/api/brand-exposure` teaser can surface "N of your lookalikes are one coordinated actor" (`clone_campaigns_for_brand` RPC). Server-side only; $0, own data.                                                                                                                                                                        |
| `cloneWatchKitPivots` (`FF_CLONE_WATCH_KIT_PIVOTS`)                               | OFF     | Registrant-intel (2026-07-17). For confirmed `likely_phishing` clones, pivot the urlscan Search API on the hosting IP to find sibling kit deployments → `attribution.kit_siblings`. Batched phase in the enricher, cap 10 searches/run, 429-aware. Server-side only; free-tier.                                                                                                                                                                                                                 |
