...
| ---------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `documentCheck`        | OFF     | Document Check Module — public `/document-check` page + `/api/document-check` + the homepage ScamChecker's PDF document mode (drawer row + drag-drop; deterministic PDF structural forensics + AU ABN pack, no paid APIs; `NEXT_PUBLIC_FF_DOCUMENT_CHECK`)                                                                                                                                      |
| `documentCheckRecords` | OFF     | Server-only: persist FLAGGED document checks as metadata-only DC- evidence records (`FF_DOCUMENT_CHECK_RECORDS`, v281, ADR-0022 pattern) + the `/document-check/[ref]` evidence page                                                                                                                                                                                                            |
| `documentCheckV1Api`   | OFF     | Server-only: the keyed B2B surface `/api/v1/document-checks` — POST per-document check (monthly-metered) + GET own-org flagged feed (`FF_DOCUMENT_CHECK_V1_API`); flip per rental-pilot key provisioning                                                                                                                                                                                        |
| `documentCheckBilling` | OFF     | Server-only: self-serve Stripe checkout for the doc plans (`doc_starter` A$29/200, `doc_pro` A$99/1,500 — separate SKU axis, never `api_keys.tier`; `FF_DOCUMENT_CHECK_BILLING`; ops: [document-check-billing-config](../ops/document-check-billing-config.md))                                                                                                                                 |
...
| `shopfrontCloneSubmitNetcraft` (`FF_SHOPFRONT_CLONE_SUBMIT_NETCRAFT`)             | OFF                       | Layer 2 Netcraft community-blocklist submission. Since the per-candidate `shopfront-clone-submit-netcraft` lane was deleted (2026-09-23) this flag's ONLY reader is `shopfront-clone-netcraft-auto`, where it is one of three gates on both its auto lane and its `/resubmit` lane (with `FF_SHOPFRONT_CLONE_NETCRAFT_AUTO` resp. `FF_CLONE_NETCRAFT_RESUBMIT`, plus `FF_SHOPFRONT_CLONE_OUTREACH`).                                                                                                                                                                                                                             |
...
| `rdapLookup` (`FF_RDAP_LOOKUP`)                                                   | **ON**                    | Registrant-intel (2026-07-17). RDAP-first domain-registration lookup in clone-watch attribution (`lookupDomainRegistration`), whoisjson fallback only when RDAP is empty. Adds EPP `statuses` (clientHold/serverHold = registrar-suspended), registrar IANA id, structured abuse contact. Server-side only; free/unmetered. OFF ⇒ whoisjson-only (byte-identical to before). ON in prod since 2026-07-17. Verification pending: its worklist (rows with `attribution IS NULL`) was empty at activation, so RDAP verifies on the next natural NRD→urlscan→enricher cycle.                                                         |
| `cloneWatchAuRegistrant` (`FF_CLONE_WATCH_AU_REGISTRANT`)                         | **ON**                    | Registrant-intel (2026-07-17). For `.au` clone alerts, fetch the auDA-disclosed registrant legal name + ABN (RDAP) and cross-check the ABN against the ABR register; cancelled/not-found/mismatched feeds `weaponisation-risk`. Stored in `attribution.au_registrant`. **Independent of `rdapLookup`** (own RDAP fetch). Legal name stored only for non-individual entities (PII guard, v236). Server-side only; free. ON in prod since 2026-07-17 but **INERT** — zero `.au` clone alerts have ever existed (the whoisds NRD feed carries no `.au` zone); blocked on an `.au` source, see #772.                                 |
| `cloneCampaigns` (`FF_CLONE_CAMPAIGNS`)                                           | **ON**                    | Registrant-intel (2026-07-17). Stamps `shopfront_clone_alerts.campaign_key` (v235; sha256 of registrar + NS roots + ASN + cert issuer) so the report card + `/api/brand-exposure` teaser can surface "N of your lookalikes are one coordinated actor" (`clone_campaigns_for_brand` RPC). Server-side only; $0, own data. ON in prod since 2026-07-17. **Verified live**: backfill 1085→585 in one run (cap 500); largest real campaign 28 domains / 18 brands.                                                                                                                                                                   |
| `cloneWatchKitPivots` (`FF_CLONE_WATCH_KIT_PIVOTS`)                               | **ON**                    | Registrant-intel (2026-07-17). For confirmed `likely_phishing` clones, pivot the urlscan Search API on the hosting IP to find sibling kit deployments → `attribution.kit_siblings`. Batched phase in the enricher, cap 10 searches/run, 429-aware. Server-side only; free-tier. ON in prod since 2026-07-17. **Verified live**: 10 rows pivoted, 86 sibling deployments from 7 searches, 3 `no_ip` sentinels.                                                                                                                                                                                                                    |
| `shopfrontClonePreclassify` (`FF_SHOPFRONT_CLONE_PRECLASSIFY`)                    | **ON**                    | PR-D2 (#498) — Haiku 4.5 pre-classifier (`shopfront-clone-haiku-preclassify`) on every new NRD candidate → `clone_watch_classifications` (v157). Its `confidence` gates the urlscan-submit / netcraft-auto / dormant / recheck worklists at `>= 0.7` and auto-triage at `>= 0.9`, and was measured to have no predictive power over outcomes (2026-08-23, 2026-09-20). Spend rolls into `SHOPFRONT_CLONE_OUTREACH_CAP_USD`.                                                                                                                                                                                                      |
| `cloneWatchJevShadow` (`FF_CLONE_WATCH_JEV_SHADOW`)                               | OFF                       | **Jev shadow lane (v311, 2026-09-21).** Sub-flag of `shopfrontClonePreclassify`: the tail of the same fn's `persist` step (`lib/clone-watch/jev-shadow-one.ts`, shared with the backfill) sends the identical three-field input to TypeSafe Jev (decision-only model, calibrated probabilities) → `clone_watch_jev_classifications`. **Read by nothing in the product path**; measured by `clone_watch_jev_calibration()`. Decision rule + delete plan: `docs/ops/clone-watch-config.md` § Jev shadow lane. Requires `TYPESAFE_API_KEY`; spend (~$0.000006/call) rolls into `SHOPFRONT_CLONE_OUTREACH_CAP_USD`.                  |
| `cloneWatchJevPrimary` (`FF_CLONE_WATCH_JEV_PRIMARY`)                             | **ON** (prod, 2026-09-22) | **ADR-0026 — Jev IS the pre-classifier.** `clone-watch-haiku-preclassify` runs one `classify-jev` step: TypeSafe Jev writes the `clone_watch_classifications` row every gate reads (`confidence` = P(clone); worklist ≥ 0.4, auto-confirm ≥ 0.8 from `lib/clone-watch/preclassify-thresholds.ts`) + the v311 raw row; no Claude call. Cost row stays `shopfront_clone_preclassify`, provider `typesafe`. OFF = rollback to Haiku (+ shadow tail).                                                                                                                                                                                |
| `cloneWatchNotACloneAuditWeekly` (`FF_CLONE_WATCH_NOT_A_CLONE_AUDIT_WEEKLY`)      | OFF                       | **Not-a-clone audit, weekly sample (#1238, v330).** The daily `shopfront-clone-urlscan-submit` lane calls `draw_clone_not_a_clone_audit_sample('weekly', …)` inside its load step: ~5% (`WEEKLY_AUDIT_FRACTION`, min 1) of the never-scanned pre-classifier `is_clone=false` pool inside 90 days, once per UTC ISO week. Samples ride the lane's existing 75-slot batch (≤ `AUDIT_SLOTS_PER_RUN` = 25), so urlscan volume does not grow. The one-off baseline sample is operator-drawn and does NOT need this flag. FN rate: `clone_watch_not_a_clone_audit_summary()`. Ops: docs/ops/clone-watch-config.md § Not-a-clone audit. |
...
| `monthlyIntelBlog` (`FF_MONTHLY_INTEL_BLOG`)                 | **ON** (prod, 2026-08-07)   | Monthly intel-driven blog draft (`monthly-intel-blog` Inngest fn, 2nd of month, 20:00 UTC). Mines the prior month across Reddit intel / competitor observations / clone-watch / regulator feeds / consumer reports → Sonnet 4.6 → Ghost draft (blog_posts fallback) + admin Telegram nudge. Never auto-publishes. Replaced the retired weekly-blog Vercel cron. Cost feature `monthly_intel_blog`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `scamBrandsSource` (`FF_SCAM_BRANDS_SOURCE`)                 | **ON in prod** (2026-07-28) | Brand-convergence Phase 1. Adds a `scam_reports` aggregation source to the existing weekly `reddit-brands-discover` run → `reddit_watchlist_candidates` gains `source='scam_reports'`. Gates ONLY the new source step; the Reddit source is unchanged when OFF. No new cron.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `brandDynamicWatchlist` (`FF_BRAND_DYNAMIC_WATCHLIST`)       | **ON in prod** (2026-07-28) | Merges the `monitored_brands` runtime overlay (v207) into the clone-watch watchlist on top of the static `AU_BRAND_WATCHLIST` array. Read through the single `getActiveWatchlist()` seam (`@askarthur/scam-engine/active-watchlist`) since v256 — **before that the merge was private to the NRD matcher, so the discovery cron, `resolve-brand`, `analyze-checkout` and `brand-register-refresh` all read the static array and were blind to overlay brands** (a promoted brand was matched by the sweep AND re-announced weekly as unwatched). Fails safe: flag off / no client / RPC error / zero rows all return exactly the static list, never an empty one. Overlay rows with no `legitimate_domains` are rejected by the seam AND by the v256 CHECK + RPC predicate — an empty exclusion list makes the matcher report a brand's own website as a clone of itself.                                                                                            |
| `brandAutoPromote` (`FF_BRAND_AUTO_PROMOTE`)                 | OFF                         | Lets the weekly `reddit-brands-discover` run promote a candidate onto the live matcher watchlist with no human in the loop, via `promote_watchlist_candidate` (v257) under the house org. Requires BOTH: the evidence bar (`scam_reports >= 2` **or** AU-hinted Reddit `>= 2`; the scam source is aggregated at >= 2 so this bar is actually reachable — while both sources shared a >= 3 threshold it was dead code) **and** a domain from a trusted store (`known_brands`). **The domain is never guessed** — `legitimate_domains` is the matcher's exclusion list, so a squatter-held `<brand>.com.au` recorded as legitimate is precisely the domain that stops being reported. Qualifying brands with no trusted domain are surfaced as "ready, needs a domain" instead. Every auto-promotion is named in the Telegram digest and reversible from `/admin/brand-candidates`. Needs `FF_BRAND_DYNAMIC_WATCHLIST` ON to have any effect on the matcher.           |
...
| `analyzeCloneCitation` (`FF_ANALYZE_CLONE_CITATION`)         | OFF                         | Brand-convergence Phase 2b. `/api/analyze` cites an operator-CONFIRMED (`tp_confirmed`/`tp_actioned`) clone-watch match of a submitted URL as a red flag (matched by `url_hash`, existing index). Only phase that changes user-facing output → canary separately. Since clone-watch deepening PR 5 it skips URLs already flagged by First-party URL Reputation (a Weaponised clone is cited once, via the verdict path), leaving the confirmed-but-not-weaponised lookalike tail as red-flag-only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `analyzeFirstPartyUrls` (`FF_ANALYZE_FIRST_PARTY_URLS`)      | **ON in prod (2026-09-23)** | Clone-watch deepening PR 5 — **First-party URL Reputation** in the analyze VERDICT on every surface (web `/api/analyze` + `runAnalysisCore` for extension and bots + `/api/extension/analyze-ad`, via `checkAnalyzeUrlReputation`) AND the extension navigation guard `/api/extension/url-check` (same seam since PR C, 2026-09-23 — it used to query `scam_urls` itself, `is_active` only; with this flag OFF url-check falls back to GSB/VT only). Looks submitted URLs up in `scam_urls` (active, high/confirmed, `feed_sources ∋ clone_watch`) in parallel with GSB/VT; a hit escalates to HIGH_RISK like a GSB hit. One indexed query (`scam_urls_normalized_url_key`), fail-open, 1.5 s bound, uncached. Keys the analysis cache when ON. Observability: Axiom always-ship warn `first_party_url_hit`; failures → `cost_telemetry.feature='first-party-url-reputation-error'` ($0, pages the health digest). Changes user-facing verdicts → canary separately. |
...
| `SHOP_SIGNAL_CAP_USD`              | `15`    | `cost_telemetry WHERE feature IN ('shop_signal', 'shop-signal-apivoid-error')` — wired by `cost-daily-check` (Stage 1 #319); the list is `BRAKE_SPEND_FEATURES.shop_signal` in `apps/web/lib/cost-brakes.ts`. See [`docs/ops/shop-signal-config.md`](../ops/shop-signal-config.md) §3 for derivation.                                                                                                                                                                                                                                                                                                                                                                                                          |
| `SHOPFRONT_CLONE_OUTREACH_CAP_USD` | `5`     | `cost_telemetry WHERE feature IN ('shopfront_clone_notify_brand', 'shopfront_clone_weekly_digest', 'shopfront_clone_urlscan', 'shopfront_clone_preclassify', 'shopfront_clone_preclassify_error', 'shopfront_clone_preclassify_jev', 'shopfront_clone_preclassify_jev_error')` — aggregate brake across brand notify, weekly digest, urlscan and the pre-classifier (Haiku + Jev, v311). The list is `BRAKE_SPEND_FEATURES.shopfront_clone_outreach` in `apps/web/lib/cost-brakes.ts` (writer-checked by `costBrakeRegistry.test.ts`); this row mirrors it. The deleted Netcraft submit/poll and urlscan-rescan tags were dropped 2026-09-24. Engages a single `feature='shopfront_clone_outreach'` brake row. |
...
- `FF_CLONE_WATCH_JEV_SHADOW` — Server-only; sub-flag of `FF_SHOPFRONT_CLONE_PRECLASSIFY` gating the Jev shadow tail (v311); only reached when `FF_CLONE_WATCH_JEV_PRIMARY` is OFF. Read with `readBoolEnv`.
- `FF_CLONE_WATCH_NOT_A_CLONE_AUDIT_WEEKLY` — Server-only; #1238 / v330 weekly not-a-clone audit draw inside the urlscan submit lane. Default OFF. Read with `readBoolEnv`.
- `FF_CLONE_WATCH_JEV_PRIMARY` — Server-only; ADR-0026. ON = Jev is the pre-classifier (no Claude call); OFF = rollback to Haiku. **ON in prod since 2026-09-22.** Read with `readBoolEnv`.
