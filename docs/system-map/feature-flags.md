...
| `ctMonitorExpanded` (`FF_CT_MONITOR_EXPANDED`)                                    | OFF     | Expands the `ct-monitor.ts` crt.sh sweep from the original `core` 9 keywords to the research-driven concentrated AU targets (super funds, Linkt, energy retailers, Macquarie/Optus/Vodafone, Medibank/Bupa, Qantas, Afterpay, NDIS, etc.). Keyword set + legit-domain exclusions derive from the shared `au-brand-watchlist` via `getCtMonitorConfig`. When OFF the **keyword sweep** is byte-identical to the pre-expansion hardcoded 9; the **legit-domain exclusion set** is now the union across all ~150 watchlist brands (a strict superset of the original 14 — strictly more conservative, can only suppress a self-cert hit, never add one). Flip ON after confirming crt.sh free-use tolerance on the 12h cadence. |
...
| Flag                                                         | Default | Purpose                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `metaBrpReport`                                              | OFF     | Meta Brand Rights Protection deepfake reporter (stub). **Not registered as of #552 — stub only**: deregistered from the Inngest function registry (no longer runs); flag + `feature_brakes` row retained for future re-registration.                                                                                                                                                                                  |
