...
| `cloneWatchJevShadow` (`FF_CLONE_WATCH_JEV_SHADOW`)                               | OFF     | **Jev shadow lane (v311, 2026-09-21).** Sub-flag of `shopfrontClonePreclassify`: the `jev-shadow` step in the same fn sends the identical three-field input to TypeSafe Jev (decision-only model, calibrated probabilities) → `clone_watch_jev_classifications`. **Read by nothing in the product path**; measured by `clone_watch_jev_calibration()`. Decision rule + delete plan: `docs/ops/clone-watch-config.md` § Jev shadow lane. Requires `TYPESAFE_API_KEY`; spend (~$0.000006/call) rolls into `SHOPFRONT_CLONE_OUTREACH_CAP_USD`.                                                      |
