...
| Route                       | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/admin`                    | Admin home — today's spend, open queues, paused brakes, per-section tiles                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `/admin/login`              | Admin login form (HMAC cookie mint via POST `/api/admin/login`); the one ungated admin route                                                                                                                                                                                                                                                                                                                                                                                                 |
| `/admin/feedback`           | Feedback triage queue review (read-only — no UI write path yet)                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `/admin/weekly-review`      | Monday signal panel — live runbook queries + `weekly_signal_log` (v270) recording; the 6-week zero-movement rule computed (#950)                                                                                                                                                                                                                                                                                                                                                             |
| `/admin/checks`             | Recent-checks viewer reconciling the homepage counter (`check_stats`) against `scam_reports`                                                                                                                                                                                                                                                                                                                                                                                                 |
| `/admin/health`             | Queue / feed / archive / Stripe-idempotency health (reads `feed_health`)                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `/admin/costs`              | Cost-telemetry dashboard                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `/admin/costs/infra`        | Per-day cloud-provider infra spend (v134) + fixed subscriptions                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `/admin/analytics`          | First-party analytics — daily scans, activation, no-scan rate, first-touch UTM/channel attribution, and per-content-page conversion. Reads the v190/v191 `security_invoker` views via service role. (No feature-flag gate — an earlier `FF_ANALYTICS_ATTRIBUTION` claim here was never enforced.)                                                                                                                                                                                            |
| `/admin/brand-alerts`       | Brand-impersonation alert review                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `/admin/brand-candidates`   | Watchlist-candidate review queue (v255) — the write surface for `reddit_watchlist_candidates.status` via `set_watchlist_candidate_status`. Ranked by `au_mention_count` (v254), not raw mentions. **Before this existed the v187 status lifecycle had no writer at all: 51 rows, 100% `pending`, zero actioned in the month from 2026-06-27.** `Worth monitoring` records a decision only — it does not add the brand to the live matcher, which needs confirmed `legitimate_domains` first. |
...
| `/admin/brand-outreach`     | Founder-composed pilot/reach-out email — worklist (RPC `get_brand_outreach_worklist`, v241) + composer + send                                                                                                                                                                                                                                                                                                                                                                                |
| `/admin/brand-stewardship`  | Monthly brand-stewardship report ledger — preview / send / outreach-done per brand-month                                                                                                                                                                                                                                                                                                                                                                                                     |
| `/admin/netcraft-results`   | Netcraft false-negative reporter results (read-only; the reporter runs on cron)                                                                                                                                                                                                                                                                                                                                                                                                              |
| `/admin/inbound-quarantine` | Inbound-email quarantine — promote to `/scam-feed` or delete (server actions)                                                                                                                                                                                                                                                                                                                                                                                                                |
| `/admin/report-card`        | Monthly clone-watch LinkedIn carousel render. **CI-critical**: the `clone-watch-linkedin` GHA renders `?slide=N&month=` headlessly via Puppeteer with a minted HMAC cookie — `SLIDE_COUNT`, the token format, and the query contract must stay in sync with `scripts/clone-watch-report-export.ts`. Deliberately absent from nav.                                                                                                                                                            |
| `/admin/partner-dashboard`  | Pilot-demo dashboard (de-identified aggregates, `?partner=` / `?jurisdiction=` framing). `FF_PARTNER_DASHBOARD`, default OFF — kept dark as the demo asset for pilot conversations (map #939, verdict #944). Not in nav.                                                                                                                                                                                                                                                                     |
| `/admin/phone-footprint`    | Phone Footprint ops panel (read-only). Feature MOTHBALLED — deliberately absent from nav + tiles; URL-reachable for the revive condition (map #939, verdict #944).                                                                                                                                                                                                                                                                                                                           |
| `/admin/showcase`           | Portfolio walk-through: interactive SVG system-architecture diagram (click a node → features / tech / engineering notes / deep link). **Fully static** — zero DB reads; all content and numbers hand-curated in `app/admin/showcase/showcase-data.ts` (guarded by `__tests__/showcaseData.test.ts`). Hash deep-links (`#analyze-engine`) pre-select a node.                                                                                                                                  |
...
| ----------------------------------- | ------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/api/document-check`               | POST   | open   | Public document check — multipart PDF upload only; structural forensics + AU content pack (ABN checksum + ABR lookup, free, `feature_brakes.document_check` skips ABR), named findings; 5/h per-IP fail-closed limit, no persistence |
| `/api/document-checks/checkout`     | POST   | auth   | Document Check plan checkout (doc_starter/doc_pro) — org-anchored, server-verified `billing:manage`; 503 `price_not_configured` until Stripe price IDs land; gated `documentCheckBilling`                                            |
...
| `/api/v1/document-checks`        | GET/POST | `apikey`            | B2B document check — POST per-document scan (monthly-metered from `documentAllowanceForOrg`: org doc-plan entitlement first, tier fallback; fail-closed meter) + GET own-org flagged records (org-scoped, never global); dark behind `documentCheckV1Api` |
...
| Route                                             | Method | Purpose                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/api/admin/login`                                | POST   | Verify `ADMIN_SECRET`, mint the `__aa_admin` HMAC cookie                                                                                                                                                                                                                                                                                                                             |
| `/api/admin/brands`                               | POST   | Pilot-brand onboarding (#953): creates a `monitored_brands` row (v207) from /admin/brand-outreach. `requireAdmin` + Zod; join key from the canonical `brandNormalize()`; rows start `verification_status='pending'` so nothing goes live unverified; 409 on the `(org_id, brand_normalized)` unique guard.                                                                           |
| `/api/admin/feeds`                                | POST   | Feed controls for /admin/health (#952): mute / unmute / disable / enable a `feed_sources` row, or dispatch a one-shot scrape probe via the scrape-feeds workflow. `requireAdmin` + Zod; one slug per request, no bulk path; always-ship warn log on every change.                                                                                                                    |
| `/api/admin/brakes`                               | POST   | Feature kill-switches for /admin/health (#951): pull or release a `feature_brakes` row. `requireAdmin` + Zod snake_case key; brake capped at 30 days; release sets `paused_until = now` (row kept, so who/why/when survives for the incident review); always-ship warn log.                                                                                                          |
