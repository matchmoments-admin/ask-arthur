...
- **Clone-watch RPCs (locked search_path, all `REVOKE EXECUTE FROM PUBLIC, anon, authenticated` unless noted; mixed DEFINER/INVOKER — the v254–v257 curation functions are INVOKER since they are service-role-only):**
  ...
  - **Not-a-clone audit (v330, #1238; SECURITY DEFINER, service_role only, function-level `statement_timeout` 30s):** `draw_clone_not_a_clone_audit_sample(p_cohort, p_label, p_size, p_fraction, p_horizon_days) → integer` (random draw of never-scanned, non-fp `detected` is_clone=false nrd alerts; idempotent per cohort key, advisory-locked, `ON CONFLICT (alert_id) DO NOTHING`); `clone_watch_not_a_clone_audit_sample_states(p_max_attempts, p_cadence_hours) → TABLE` (the ONE definition of a sample's state — scanned / in_flight / due / waiting / unscannable — read by both of the next two); `list_clone_not_a_clone_audit_pending(p_limit) → TABLE` (`due` samples, never-tried first); `mark_clone_not_a_clone_audit_attempted(bigint[]) → integer` (attempts+1); `claim_clone_not_a_clone_audit_misses(p_limit) → TABLE` (each miss once, for the lane's warn); `clone_watch_not_a_clone_audit_summary(p_since) → TABLE` (per cohort key × classifier × age band: sampled / attempted / scanned / pending / unscannable / misses / fn_rate / per-verdict counts / phishing_later / weaponised_later — the #1237 scorecard input). v330 also re-creates `apply_clone_urlscan_verdict` (a `likely_phishing` verdict on a sampled, still-is_clone=false alert → `monitoring` + `miss_at`, never `weaponised`) and `list_clone_alerts_for_recheck` (from v328, `due_total` kept; sampled not-a-clones on a weekly cadence clocked from their audit scan).
