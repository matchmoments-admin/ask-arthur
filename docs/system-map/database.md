...
- `project_clone_to_platform_entity(p_alert_id, p_scam_url_id DEFAULT NULL) → boolean` + trigger `clone_alert_platform_projection` (AFTER UPDATE OF attribution, lifecycle*state on `shopfront_clone_alerts`, WHEN the alert carries `submitted_to.platform_entity`) — **v315**. Keeps a fed clone's Platform Entity current: `attribution.whois` → `scam_urls.whois*\*`(COALESCE) and taken_down/dormant/expired (sole source`clone_watch`) → `confidence_level`medium. Backfill 2026-09-23: 138/164 fed rows gained registrar data, 25 downgraded. **v320** (SQL twin of`readAttribution`): list-valued registrar → first entry, unparseable dates → NULL (never raise inside the trigger), `createdDate`> 1 year before the alert's`first_seen_at`→ NULL (parent-zone date; 7 rows backfilled), weaponised again → medium→high +`is_active`, skips a retracted entity. v315 also moved the `p_min_confidence`DEFAULT of`list_clone_alerts_pending_urlscan_submit`, `mark_stale_clone_alerts_dormant`, `list_clone_alerts_pending_netcraft_auto`0.7 → 0.4 (ADR-0026; pinned by`preclassifyThresholds.test.ts`).
...
- `reddit_watchlist_candidates` — Review queue of brands seen impersonated but NOT yet on the clone-watch watchlist. `brand_normalized UNIQUE` (the `public.brand_normalize()` key), `raw_brand`, `mention_count` (denormalised sum of `source_counts` jsonb), `resolved_canonical`, `status` (`pending`|`reviewed`|`dismissed`|`promoted`), `first_seen_at`/`last_seen_at`. Service-role RLS. v187; multi-source `source_counts` v196. **v254** adds `au_counts jsonb` + `au_mention_count int` — the AU-attributable share, from `reddit_post_intel.country_hints`, because the source corpus is ~93% non-AU and ranking by raw mentions ranked by "how American is this brand". Invariant `au_mention_count <= mention_count` is enforced by the upsert's `LEAST()` clamp, not by convention. Partial index `idx_rwc_pending_au`. **v255** adds `status_changed_at` + `status_note` and widens the status CHECK to include `promoted`; before v255 the v187 status lifecycle had NO writer anywhere in the codebase (51 rows, 100% `pending`, one month).
...
### Operational alerting (v263–v264, applied to prod as v260–v261)

| Table / view                | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `alert_delivery_log` (v263) | One row per operator-alerter firing, **including the no-issue case**. A missing row means the alerter did not run — never that nothing was wrong. Service-role only. ~4,100 rows/week across the 8 alerters. Written via `apps/web/lib/alerting/deliveryLog.ts`.                                                                                                                                                       |
| `feed_sources` (+v264/v262) | Feed roster. `enabled` drives which feeds are health-checked; `expect_new_rows_days` opts a feed into silent-success detection; `muted_until` / `muted_reason` are a **dated** alarm mute; `staleness_exempt` (v262) marks a historical-signal feed whose findings must not expire on the 7-day freshness clock. Note: `last_success_at` / `consecutive_failures` have **no writer** — do not read them for freshness. |
| `feed_health` (view, v264)  | One row per ENABLED feed **whether or not it has logged anything** — the LEFT JOIN is what makes an absent feed detectable. `NULL last_success_at` means never succeeded and must be read as infinitely stale. Unions `feed_ingestion_log` + `vulnerability_ingestion_log`. Sole consumer: `health-digest`.                                                                                                            |
...
- `count_todays_takedown_submissions() → int` — the shared daily-cap counter (`CLONE_SUBMISSION_DAILY_CAP`) read by `shopfront-clone-enforcement-execute` (fails CLOSED on error) and the admin clone-enforcement send route. Sums today's `cost_telemetry` `clone_enforcement` `enforcement.queued` + `enforcement.reported` units — the sends under our one email identity. **v320** removed the dead `shopfront_clone_submit_netcraft` term; Netcraft is deliberately outside this cap (its own caps: auto-lane worklist RPC, `NETCRAFT_RESUBMIT_DAILY_CAP`, `count_todays_netcraft_issues`). v205 → v318 → v320. Service-role only.
- `onward_report_log` — THE onward reporting ledger (v119): one row per report attempt to a destination (Scamwatch / ACMA / IDCARE / brand abuse / OpenPhish / APWG). Since v318 two sources (`source`): `scam_report` (user click, bots, `report-onward-auto-report`) and `clone_alert` (`shopfront-clone-enforcement-execute`; `clone_alert_id` FK ON DELETE SET NULL). Dedup: v119 `(scam_report_id, destination, destination_key)` + v318 per-URL `(destination, destination_key, url_key)` where `url_key = onward_url_key(url)`. URL-blocklist rows are written only by `enqueue_onward_url_reports`; clone worklist `list_clone_alerts_pending_onward`. The URL-blocklist worker claims a row `queued → sending` before sending (a duplicate event finds nothing to claim) and marks `failed` when the send exhausts its retries. `onward_destination` enum gained `netcraft` (v318, ledger-only — no worker yet). Service-role RLS.
...
- `shopfront_clone_alerts` — Single write target for ALL clone detections (ADR-0016 Decision #1). Layer 0 writes `target_shop_id IS NULL`, `source = 'nrd'`. CHECK enforces XOR on `target_shop_id` / `inferred_target_domain`. `signals` JSONB array per ADR-0015. UNIQUE expression index on `(COALESCE(target_shop_id::text, inferred_target_domain), url_hash)`. Service-role RLS. v140. **Extended in v143** with triage columns: `triage_status` (CHECK in `'pending'|'tp_confirmed'|'fp'|'needs_investigation'|'tp_actioned'`), `triage_by uuid`, `triage_at timestamptz`, `triage_notes text`, `submitted_to jsonb DEFAULT '{}'::jsonb`. **Extended in v148** with urlscan columns: `urlscan_evidence jsonb`, `urlscan_classification` (CHECK in `'parked_for_sale'|'unresolved'|'likely_phishing'|'neutral'`), `urlscan_scanned_at timestamptz`, `urlscan_uuid text`. Partial indexes: `idx_clone_alerts_triage_pending`, `idx_clone_alerts_urlscan_rescan`. **Extended 2026-07-17:** `campaign_key text` (v235 — coarse actor fingerprint computed in TS by `campaign-fingerprint.ts`, sentinel `'insufficient'`; partial index excluding null/insufficient) + `weaponised_notified_at timestamptz` (v236 — durable weaponised.v1 emit worklist: `weaponised_at NOT NULL AND weaponised_notified_at NULL`, partial index). `attribution` jsonb additively carries `whois.{statuses,registrarIanaId,abuseContact,source}`, `au_registrant`, `kit_siblings` (all flag-gated, no migration). **v329 (#1234):** `liveness_checked_at timestamptz` (cadence stamp of the weaponised DNS sweep) + `liveness_last_verdict text` (present / gone / inconclusive) + `offline_since timestamptz` (first NXDOMAIN of a weaponised alert; confirmed by a second ≥ 12 h later, which moves it to `dormant`; cleared if it re-emerges) + `offline_cause text` (`registrar_hold` / `nxdomain`). There is deliberately no `taken_down_at` column: the Netcraft takedown clock already lives in `submitted_to.netcraft.takedown_at` (vendor-dated since v314), and lifecycle `taken_down` means "Netcraft classified it malicious" to every reader, so "went offline" is `dormant` + `offline_since`, not `taken_down`. **v336 (#1253):** `attribution_retry_after timestamptz` (partial index where non-null) — set when the dossier's WHOIS lookup got no answer (whoisjson monthly guard / non-200 / no key → `attribution.whois.source = 'deferred'` + `retryAfter`); the enricher's `whois-reoffer` step re-runs only the registration lookup once it is due and clears it (answered / given up after 3 http errors) or pushes it forward. Written by `apply_clone_alert_attributions` (v331 body + the column) and `apply_clone_alert_whois_reoffers(jsonb)` (merges ONLY `attribution.whois`, only rows still due — `attribution_retry_after <= now()` — so a stale write cannot clobber a row pushed forward).
- `shopfront_takedown_attempts` — the HUMAN-GATED enforcement case workflow per clone alert (v140, evolved to cases in v201): GSB / SmartScreen deep-links and registrar / hosting abuse with four-eyes approval. **Deprecated as a send ledger (v318):** auto APWG/OpenPhish sends are `onward_report_log` rows and open no case here; the admin registrar/hosting send still records here (follow-up). 0 rows on 2026-09-23. Service-role RLS.
...
- `monitored_brands` — Runtime overlay merged into the clone-watch watchlist on top of the compile-time `AU_BRAND_WATCHLIST` array. Org-scoped for paid Brand Monitor / pilots (v207), plus brands Ask Arthur watches on its own evidence under the reserved **house org** `00000000-0000-4000-8000-000000000001` (v256 — `org_id` has no FK and there is no `orgs` table, so this is a sentinel, and the v207 RLS read policy resolves membership via `org_members`, making house rows service-role-only by construction). `UNIQUE(org_id, brand_normalized)`. Only `is_active AND verification_status='verified'` rows are monitored. **v256 CHECK `verification_status <> 'verified' OR cardinality(legitimate_domains) > 0`** — a brand may be REGISTERED without confirmed domains but never MONITORED, because `legitimate_domains` is the matcher's EXCLUSION list and an empty one makes the brand's own website match as a clone of itself. Read through the single `getActiveWatchlist()` seam (`@askarthur/scam-engine/active-watchlist`), gated `FF_BRAND_DYNAMIC_WATCHLIST`.
...
- `clone_watch_not_a_clone_samples` — **Not-a-clone audit sample** (v330, #1238). One row per sampled alert (PK `alert_id`, FK → `shopfront_clone_alerts` ON DELETE CASCADE; an alert is sampled at most once): `cohort` (`baseline` | `weekly`), `cohort_key` (`baseline:<label>` | `weekly:<IYYY>-W<IW>`), `sampled_at`, the classifier snapshot at draw (`model_id`, `confidence`, `classified_at`, `triage_status`), the alert's `first_seen_at`, `pool_size`, the attempt ledger (`attempts`, `last_attempt_at` — written by the urlscan submit lane) and the miss record (`miss_at` — written by `apply_clone_urlscan_verdict`; `miss_warned_at` — the lane logged it). The FN rate is DERIVED: `clone_watch_not_a_clone_audit_summary()` reads the first post-draw verdict from `clone_watch_scan_transitions`. Reporting reads it as a PostgREST to-one embed on `shopfront_clone_alerts` (`AUDIT_SAMPLE_EMBED`, `apps/web/lib/clone-watch/clone-cohort.ts`). Brand counts withhold a sample's urlscan facts until it is re-judged `is_clone=true` (#1256). RLS on, service_role-only policy, no anon/authenticated grants.
- `clone_watch_readiness` — **Readiness scorecard** (v335, #1237). One row per closed month (PK `period_month`, first-of-month CHECK): for each of seven components (`precision`, `fp_share`, `fn_rate`, `lane_health`, `report_diff`, `takedown`, `stock`) a `_value`, `_n`, `_threshold` (the threshold in force when computed) and `_status` (`pass` / `fail` / `insufficient`), plus `ready`, `detail` jsonb (reason lines, secondary thresholds, context) and `computed_at`. CHECK `ready_iff_all_pass`: `ready` is true exactly when all seven pass. NULL value = not measured, never 0. Written by `clone-watch-report-summary` (step `compute-readiness`, upsert); read by the brand SEND gate (`readReadinessGate`) and `/admin/clone-watch`. Inputs RPC `clone_watch_readiness_inputs(p_start, p_end)` (SECURITY DEFINER, 30 s fn-level timeout) returns human DECIDED verdicts (tp_confirmed/tp_actioned/fp; origin by `shopfront_clone_alerts.triage_source`, or for pre-v335 rows no machine note marker `auto-park%` / `auto-triage%` / `[matcher-v4-audit]%`), classifier reject share, and health-digest lane-health days from `alert_delivery_log` (a day is measured only when its row carries a `lane_problems` array). RLS on, no policies, service_role only.
- `shopfront_clone_alerts.triage_source` (v335, #1237) — `'human'` | `'machine'` | NULL (pre-v335): who set the current `triage_status`, recorded apart from `triage_notes` (which `set_clone_alert_triage` COALESCEs, so a note never proves origin). Stamped by `set_clone_alert_triage(…, p_source DEFAULT 'human')` (v335 re-created it; the 4-arg overload was dropped) and by `auto-park.ts` (`'machine'`). `merge_clone_alert_submission` stamps `'machine'` on any status it CHANGES except tp_confirmed→tp_actioned, which keeps the human origin (v338, #1263 — the v335 header's "not stamped" was wrong: the Netcraft auto lane actions any non-fp status, so a human needs_investigation it submitted read as a human TP). Not stamped by `persist_clone_alert_urlscan` (only suggests needs_investigation, and never over a decided status).
- `clone_watch_monthly_brand_stats` / `clone_watch_monthly_registrar_stats` — **the monthly per-brand store** (v193; outcome columns v218; targeting v296; v319). Grain `(period_month, brand)` where `brand` is the brand's PRIMARY DOMAIN; `brand_normalized` (v319) is the Canonical Brand key beside it (ADR-0020, 2026-09-23 amendment). v319 adds `weaponised_ever` (vs `weaponised` = current state), `taken_down_in_month` (EVENT-dated from `submitted_to.netcraft.takedown_at`, vs `taken_down` = first-seen cohort), `weaponised_after_decline`, `re_taken_down`, `alert_ids` (cohort membership) and `frozen_at` (NOT NULL = published). ONE writer: `write_clone_watch_monthly_stats` (called only by `clone-watch-report-summary`); a frozen month is never rewritten unless `p_republish`, and the `clone_watch_brand_stats_freeze_guard` trigger refuses direct UPDATE/DELETE/INSERT on a frozen month. Readers: report card trend gate, targeting intelligence, monthly intel blog, brand coverage, brand stewardship. Deny-all RLS. **v325** adds the STOCK view beside the flow: `active_stock_eom` / `stock_by_status` / `liveness_checked_at` (from `clone_liveness_snapshots`; NULL = no month-end snapshot, never 0), `new_registered` / `new_deliberate`, `swept_domains` (NRD feed denominator), `coverage_full_month`, `matcher_version`, `classifier_version`; and a ZERO row (`clones = 0`) for every brand watched in the month — the ledger read and the blog filter `clones > 0`.
- `clone_liveness_snapshots` — month-end DNS liveness of every active-stock lookalike, one row per `(period_month, alert_id)`; status `live_phishing | live | parked | held | no_host | gone | unverified` (rule: `clone-metrics.ts` `stockStatus`). Writer `clone-watch-month-end-liveness`, reader `clone-watch-report-summary` (v325). Deny-all RLS, service_role only.
- **Clone-watch RPCs (locked search_path, all `REVOKE EXECUTE FROM PUBLIC, anon, authenticated` unless noted; mixed DEFINER/INVOKER — the v254–v257 curation functions are INVOKER since they are service-role-only):**

  _Watchlist curation + brand overlay (v254–v257) — all service-role only:_
  - `aggregate_reddit_brands_with_au(p_since TIMESTAMPTZ, p_min_count INT) → TABLE` — windowed brand-mention aggregate over `reddit_post_intel` with the AU-hinted subset counted separately. Groups on `public.brand_normalize()` (the SQL twin of the TS `brandNormalize()`) so the join key cannot desync from `brand_aliases`. Counts DISTINCT posts, not array entries. v254.
  - `upsert_watchlist_candidate(p_brand_normalized, p_raw_brand, p_source, p_source_count, p_au_count, p_resolved_canonical)` — source-aware, status-preserving upsert; recomputes both denormalised sums; clamps `au_count <= source_count`. **6-arg overload added in v254; the v196 5-arg form is deliberately left in place** because migrations are applied to prod BEFORE the code that uses them merges, so dropping the old signature would break the deployed weekly run in the gap.
  - `set_watchlist_candidate_status(p_brand_normalized, p_status, p_note) → INT` — the ONLY writer of `status`. Returns rows changed (0 = unknown brand or already in that state) so a caller can tell a no-op from success; no-ops when nothing would change so `status_changed_at` keeps meaning "when the decision was made". v255.
  - `promote_watchlist_candidate(p_brand_normalized, p_brand_name, p_domains, p_aliases, p_note, p_source) → TABLE` — **atomic**: writes `monitored_brands` under the house org AND moves the candidate to `promoted` in one transaction. Split apart, a failure between them leaves a brand that is monitored and simultaneously re-announced weekly as unwatched. Refuses an empty domain list outright (usable error rather than the v256 CHECK's opaque 23514) and normalises domains server-side to match the TS matcher. v257.
  - `demote_watchlist_candidate(p_brand_normalized, p_note) → INT` — one-call undo: deactivates the house-org row (does NOT delete — decision history survives) and returns the candidate to `pending`. v257.
  - `list_active_monitored_brands() → TABLE` — matcher worklist: verified + active overlay brands **with at least one legitimate domain** (domain guard added v256, duplicating the CHECK on purpose — a single guard is one ALTER away from the self-clone bug). v207/v256.
  ...
  - `clone_watch_takedown_stats(p_days INT) → TABLE` — v145, **rewritten v329 (#1234)**; service-role only (v160). Every duration now has both ends on ONE event pair: `median/p90/fastest/slowest_minutes` = Netcraft triage latency on Netcraft's own clock (`takedown_received_at → takedown_at`, sample `timed_n`, NULL — not 0 — when empty); `detect_to_block_*` = our `weaponised_at` → vendor-dated `takedown_at`, with sites Netcraft blocked before we saw phishing counted in `blocked_before_detection`, never averaged; `weaponised_*` = the cohort weaponised in the window by current outcome (blocklisted / offline / open / open-with-no-threat-verdict / escalated) + `detect_to_offline_median_minutes`. `takedowns_total` = Netcraft classifications dated in the window. The v145 form subtracted OUR `submitted_at` from Netcraft's time and read median 0, fastest −2 min (2026-09-26). Read ONLY through `apps/web/lib/clone-watch/takedown-stats.ts`.
  ...
  - `write_clone_watch_monthly_stats(p_period_month DATE, p_brand_rows JSONB, p_registrar_rows JSONB, p_republish BOOL DEFAULT false) → JSONB` — the ONE writer of the monthly per-brand + per-registrar store: atomic replace of a month under a per-month advisory lock, freezing it (`frozen_at = now()`); returns `{status: written|republished|frozen, frozen_at, previous_frozen_at, brand_rows, registrar_rows}` and writes NOTHING for a frozen month unless `p_republish`. SECURITY DEFINER, function-level `statement_timeout = 60s`, service-role only. v319.
  ...
  - **Not-a-clone audit (v330, #1238; SECURITY DEFINER, service_role only, function-level `statement_timeout` 30s):** `draw_clone_not_a_clone_audit_sample(p_cohort, p_label, p_size, p_fraction, p_horizon_days) → integer` (random draw of never-scanned, non-fp `detected` is_clone=false nrd alerts; idempotent per cohort key, advisory-locked, `ON CONFLICT (alert_id) DO NOTHING`); `clone_watch_not_a_clone_audit_sample_states(p_max_attempts, p_cadence_hours) → TABLE` (the ONE definition of a sample's state — scanned / in_flight / due / waiting / unscannable — read by both of the next two); `list_clone_not_a_clone_audit_pending(p_limit) → TABLE` (`due` samples, never-tried first); `mark_clone_not_a_clone_audit_attempted(bigint[]) → integer` (attempts+1); `list_clone_not_a_clone_audit_unwarned_misses(p_limit) → TABLE` (read-only) + `mark_clone_not_a_clone_audit_misses_warned(bigint[]) → integer` (stamped only AFTER the lane's Axiom warn and Outcome Row `audit_miss_ids`); `clone_watch_not_a_clone_audit_summary(p_since) → TABLE` (per cohort key × classifier × age band: sampled / attempted / scanned / pending / unscannable / misses / fn_rate / per-verdict counts / phishing_later / weaponised_later — the #1237 scorecard input). v330 also re-creates `apply_clone_urlscan_verdict` (a `likely_phishing` verdict on a sampled alert not explicitly re-judged `is_clone=true` → `monitoring` + `miss_at`, never `weaponised`; fails closed on a NULL/missing classification) and `list_clone_alerts_for_recheck` (from v328, `due_total` kept; sampled not-a-clones on a weekly cadence clocked from their audit scan).
...
## RPCs (177 public functions as of 2026-07-29 — 136 `SECURITY DEFINER`, 41 `SECURITY INVOKER`)

> Counted live: `select count(*) filter (where prosecdef), count(*) filter (where not prosecdef) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prokind='f'`. The previous "90 total — all SECURITY DEFINER" was stale on both figures. **DEFINER is not the default** — prefer INVOKER for service-role-only functions and reserve DEFINER for the cases that genuinely need to cross an RLS boundary (e.g. `list_active_monitored_brands`, which the matcher calls to read org-scoped rows).
...
| `clone_watch_brand_stats_freeze_guard` | `clone_watch_monthly_brand_stats` | `clone_watch_brand_stats_freeze_guard()` | Refuse UPDATE/DELETE/INSERT on a frozen (published) month unless the v319 writer lifted it for its own transaction (v319) |
...
- `record_clone_watch_jev_classification(16 params) → VOID` — idempotent UPSERT on `alert_id`; called only via `classifyOneWithJev` (`apps/web/lib/clone-watch/jev-shadow-one.ts`) — the one write path used by the `persist` step of `clone-watch-haiku-preclassify` (source `live`) and `scripts/backfill-jev-classifications.ts` (source `backfill`). DEFINER, `search_path=''`, `statement_timeout='30s'`.
- `clone_watch_jev_calibration() → TABLE(classifier, bucket, n, urlscan_phish, weaponised, netcraft_declined, triaged_fp, tp_actioned)` — the decision instrument: Haiku (v157 `confidence`, `is_clone=false` → bucket 0) vs Jev (`is_clone_p`) in ten decile bins over the alerts BOTH scored (edges fixed in **v312**: numeric rounding + `LEAST(…,10)`), with outcome counts joined from the parent. Decision rule + delete plan in `docs/ops/clone-watch-config.md` § Jev shadow lane. Both RPCs are covered by `rpcs.smoke.test.ts`. **v313** restricts the haiku side to `model_id NOT LIKE 'jev%'` because since ADR-0026 (2026-09-22) Jev writes `clone_watch_classifications` directly — `confidence` there is now P(clone), `model_id` tells the eras apart. That filter sits in the CTE both sides share, so the function is **frozen to the pre-swap cohort** (it cannot see a primary-mode alert) — documented in its COMMENT by **v314**.
- `clone_watch_preclassify_calibration(p_since timestamptz DEFAULT NULL) → TABLE(model_id, bucket, n, urlscan_phish, weaponised, netcraft_declined, triaged_fp, tp_actioned)` — **v314**, the LIVE instrument (ADR-0026 30-day revisit): one calibration curve per `model_id` over the gate rows (`clone_watch_classifications.confidence`), uniform decile buckets, grouped not prefix-filtered so a renamed vendor model appears as a new group.
- **v329 (#1234) — weaponised outcome observation**, all service-role only, called by `shopfront-clone-netcraft-reconcile`: `list_weaponised_for_liveness(p_limit, p_cadence_hours, p_dormant_cadence_hours) → (id, candidate_domain, lifecycle_state, due_total)` (every `weaponised` row daily and every offline `dormant` clone — `offline_since` + `weaponised_at` set — weekly; no age cap, NULL-first; `due_total` counted before the LIMIT) + `record_weaponised_liveness(p_results jsonb, p_confirm_hours) → (checked, present, gone_unconfirmed, offline_confirmed, inconclusive, re_emerged)` (first NXDOMAIN sets `offline_since`; a second ≥ 12 h later moves weaponised → `dormant`/`expired` with `offline_cause` = `registrar_hold` when the stored RDAP statuses show a hold, else `nxdomain`; an offline dormant clone that resolves again goes back to `weaponised`/`open`; every read sets `liveness_last_verdict`). Resubmit lane: `list_clone_alerts_pending_netcraft_resubmit` (v253 body) now excludes `netcraft_vendor_gap_basis = 'rejected'`, and `count_netcraft_resubmit_rejected()` counts the exclusion. No-threat-on-phishing: `netcraft_vendor_gap_basis(submitted_to, p_min_issue_age_hours) → 'rejected' | 'issue_unanswered' | NULL` (the ONE predicate), `list_netcraft_vendor_gap(...)` (read) and `mark_netcraft_vendor_gap_escalated(ids, ...)` (stamps `submitted_to.vendor_gap` once per alert, re-checking the predicate) — list → operator page → mark, so a lost page is never a lost escalation.
- `record_netcraft_url_verdicts(p_verdicts jsonb) → int` — **v329** also persists Netcraft's receipt time: `submitted_to.netcraft.received_at` (current uuid) and `takedown_received_at` beside a vendor-dated `takedown_at` (same submission), the start of the Netcraft-clock triage latency. **v314**, called by `shopfront-clone-netcraft-reconcile` before `apply_netcraft_reconcile`: persists Netcraft's per-URL `url_state` / `url_state_reason` / `url_state_at` under `submitted_to.netcraft`, stamps `takedown_at` from Netcraft's own `classification_log` (`takedown_at_source='netcraft_log'`) when it is on/after the submission, else flags `already_malicious_at_submit`. v316 adds `unchanged_reads` (reconcile backoff input); **v320** makes it idempotent — it increments only when the stored `url_state_at` is > 1 h old, so a retried apply step does not double-count. Service-role only.
