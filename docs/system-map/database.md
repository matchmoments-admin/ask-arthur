...
- `document_check_records` — Metadata-only evidence records for FLAGGED document checks (DC- check_ref, doc_sha256, curated structural_summary, findings, abn_summary, org_id/api_key_hash for B2B attribution — never bytes, never extracted text; ADR-0022 pattern). v281. Written only by `recordDocumentCheck` (`apps/web/lib/document-check-records.ts`), gated `FF_DOCUMENT_CHECK_RECORDS`.
