...
- **v329 (#1234) — weaponised outcome observation**, all service-role only, called by `shopfront-clone-netcraft-reconcile`: `list_weaponised_for_liveness(p_limit, p_cadence_hours) → (id, candidate_domain, due_total)` (every `weaponised` row, no age cap, NULL-first; `due_total` counted before the LIMIT) + `record_weaponised_liveness(p_results jsonb, p_confirm_hours) → (checked, present, gone_unconfirmed, offline_confirmed, inconclusive)` (first NXDOMAIN sets `shopfront_clone_alerts.offline_since`; a second ≥ 12 h later moves weaponised → `dormant`/`expired`; a resolving read clears it; null only stamps `liveness_checked_at`). No-threat-on-phishing: `netcraft_vendor_gap_basis(submitted_to, p_min_issue_age_hours) → 'rejected' | 'issue_unanswered' | NULL` (the ONE predicate), `list_netcraft_vendor_gap(...)` (read) and `mark_netcraft_vendor_gap_escalated(ids, ...)` (stamps `submitted_to.vendor_gap` once per alert, re-checking the predicate) — list → operator page → mark, so a lost page is never a lost escalation.
