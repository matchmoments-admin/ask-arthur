...
| **Public read, service write** | `scam_reports`, `verified_scams`, `feed_items`, `acnc_charities` | `USING(true)` for SELECT. INSERT/UPDATE/DELETE restricted to service role. Write-once data. **`scam_reports` + `scam_entities` + `report_entity_links` anon Public-read scheduled for removal in v172 (#566)** — closes a bulk-export hole (raw scammer phone/email leaked via `/rest/v1/scam_entities?select=*`); after removal readers use service_role / the rate-limited `/api/scam-contacts/lookup`. **v172 pending prod apply (HELD).** `feed_items` + `acnc_charities` are unaffected. |
