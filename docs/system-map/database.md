...
- **Clone-watch RPCs (locked search_path, all `REVOKE EXECUTE FROM PUBLIC, anon, authenticated` unless noted; mixed DEFINER/INVOKER — the v254–v257 curation functions are INVOKER since they are service-role-only):**
  ...
  - **Not-a-clone audit (v330, #1238; SECURITY DEFINER, service_role only, function-level `statement_timeout` 30s):** `draw_clone_not_a_clone_audit_sample(p_cohort, p_label, p_size, p_fraction, p_horizon_days) → integer` (random draw of never-scanned `detected` is_clone=false nrd alerts; idempotent per cohort key, advisory-locked); `list_clone_not_a_clone_audit_pending(p_limit) → TABLE` (the submit lane's audit worklist — un-stamped, no uuid, no classification, no urlscan attempt since the draw); `mark_clone_not_a_clone_audit_attempted(bigint[]) → integer`; `clone_watch_not_a_clone_audit_summary(p_since) → TABLE` (sampled / attempted / scanned / pending / unscannable / misses / fn_rate / weaponised_later per cohort key × classifier — the #1237 scorecard input).
