...
### Operational alerting (v260–v261)

| Table / view                | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `alert_delivery_log` (v260) | One row per operator-alerter firing, **including the no-issue case**. A missing row means the alerter did not run — never that nothing was wrong. Service-role only. ~4,100 rows/week across the 8 alerters. Written via `apps/web/lib/alerting/deliveryLog.ts`.                                                                                                                                                       |
| `feed_sources` (+v261/v262) | Feed roster. `enabled` drives which feeds are health-checked; `expect_new_rows_days` opts a feed into silent-success detection; `muted_until` / `muted_reason` are a **dated** alarm mute; `staleness_exempt` (v262) marks a historical-signal feed whose findings must not expire on the 7-day freshness clock. Note: `last_success_at` / `consecutive_failures` have **no writer** — do not read them for freshness. |
| `feed_health` (view, v261)  | One row per ENABLED feed **whether or not it has logged anything** — the LEFT JOIN is what makes an absent feed detectable. `NULL last_success_at` means never succeeded and must be read as infinitely stale. Unions `feed_ingestion_log` + `vulnerability_ingestion_log`. Sole consumer: `health-digest`.                                                                                                            |
