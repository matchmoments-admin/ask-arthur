...
| **Clone Alerts** — daily scan against existing tables (`scam_reports` + `reddit_post_intel` + `feed_items`) keyed off the Verified Shop's brand-name / handle / domain — surfaces Brand Matches (corpus permutation matching) + Visual Matches (logo pHash). Signal taxonomy + per-phase application codified in **ADR-0015**; data-source layering (corpus → CT firehose → NRD + Hetzner) codified in **ADR-0016** | Brand-protection intelligence (currently $20K-200K/yr enterprise tools); manual takedown templates | Merchants become an active reporting surface for new clone scams                                      |
...
- `packages/shopfront-glue/` — typed wrappers around clone-detection scanners, billing API hooks, takedown-template assembly, continuous-verification scheduler. Thin orchestration over `scam-engine`; ≤2K LOC. **Phase A + Phase B clone-detection both live here.** A standalone `packages/domain-monitor/` is explicitly NOT introduced at Phase A or B — it would be a pass-through wrapper that fails the deletion test. See ADR-0016 for the source-layering rationale.
...
| **Clone-detection scanners** — Brand Match (corpus permutation matching) + Visual Match (logo pHash) + JSON-LD product clone + Shopify theme fingerprint. Pull-mode corpus query, no new scrapers added | 2.0                |
...
#### Stage 2 (a) Clone-detection: CT firehose add-on (Phase B) (~1–2 eng-weeks)

Pre-condition: ≥10 paying Shield Pro merchants from Stage 1 AND a 48-hour Calidog public-WSS stability spike returning ≥99% uptime (the spike is the **first sub-task** of this Phase, runs before any build code commits).

- Subscribe to Calidog public certstream WSS as a sibling data source feeding the SAME `shopfront_clone_alerts` table from Phase A — NOT a parallel `clone_findings` table. Source-layering rationale in ADR-0016.
- New `shopfront_shop_permutations` table: per-`shopfront_shops` row, precomputed Levenshtein + Unicode-confusable + punycode domain permutations via dnstwist. Refreshed by a daily Inngest cron when shops are added/removed.
- Aho-Corasick matcher over the brand-name corpus + dnstwist permutations, fed by the Calidog WSS stream. NO Voyage embeddings at this Phase (per ADR-0015 signal model).
- Lives in `packages/shopfront-glue/` (extended), NOT a new `packages/domain-monitor/` (per ADR-0016 — the deletion test only passes for that package at Phase C).
- Cost telemetry: `feature: 'shopfront_clone_scan'` (existing) — Calidog itself is free; matcher CPU cost is bounded by `feature_brakes.shopfront_clone_scan`.
- **48-hour spike failure mode:** if Calidog uptime <99% over the window, Phase B inherits the Hetzner self-host work from Phase C and the eng-week estimate jumps from 1–2 weeks to 4–5 weeks. Captured under risk 10.

#### Stage 2 (b) Clone-detection: Voyage + NRD (Phase C / Layer 4) (~3–4 eng-weeks)

Pre-condition: Phase B shipping + **#368 SPF-sector WTP validation gate landing positive** (per ADR-0012) + **#369 privacy counsel opinion** for the cross-merchant clustering surface this Phase enables. Drives the Layer 4 Threat Feed License product (#372).

- Add Voyage `voyage-3` page-content embeddings as the **primary verdict** for the "logo-swap, copy-preserved" attack class (per ADR-0015) — the harder ~30% of clones where deterministic-visual misses (new logo) AND deterministic-string misses (rephrased copy) but the page semantically mirrors the verified merchant. NOT a uniform confidence-booster on top of deterministic signals; only adjudicates candidates where the deterministic verdict scored below ship-threshold.
- Apply the sibling-table pattern: new `shopfront_clone_alerts_embeddings` table (1:1 with `shopfront_clone_alerts`) carries the Voyage column + HNSW index + `<col>_embedding_model_version TEXT` stamp (per ADR-0003 + ADR-0004 + ADR-0005 + CLAUDE.md Critical Rules). `shopfront_clone_alerts` itself stays lean.
- Add whoisds NRD (Newly Registered Domains) daily backstop as a fall-through source for candidates that neither corpus nor Calidog surfaced but match a brand-permutation lookup.
- Self-host Hetzner certstream-server **only if** the Phase B Calidog stability spike failed OR an enterprise Layer 4 customer's SLA requires it. Default position: stay on Calidog.
- Lives in a new `packages/domain-monitor/` package — at THIS Phase the deletion test passes (Hetzner + NRD + Voyage adapters + sibling-table maintenance are independently complex enough to justify a separate package).
- Cost telemetry: `feature: 'shopfront_clone_embed'` (new) tagged with `provider: 'voyage'` per embedding call; new `feature_brakes.shopfront_clone_embed` row.
...
10. **Calidog public certstream-server reliability** (Stage 2 (a) / Phase B clone-detection per ADR-0016). The public Calidog WSS endpoint has had documented outages and the maintainer has flagged cost pressure on running the public service. Phase B inherits a 48-hour stability spike as its **first sub-task** before any build code commits — pass threshold ≥99% uptime over the window. If the threshold fails, Phase B inherits the Hetzner self-host work from Phase C, eng-week estimate jumps from 1–2 weeks to 4–5 weeks, and the Phase C gating on enterprise customer demand applies only to the NRD + Voyage additions, not to Hetzner. If Calidog announces a paywall / shutdown post-launch, Phase B re-targets Hetzner immediately and a follow-up ADR captures the operational ownership of the self-hosted stream.
