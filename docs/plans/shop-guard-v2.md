...
- New env var `APIVOID_API_KEY` + `SHOP_GUARD_CAP_USD=15`
...
- `cost_telemetry` row per call, tagged `feature='shop-signal'`, `provider='apivoid'`
...
| Stage 1 baseline   | A$15 (`SHOP_GUARD_CAP_USD=15`) | ~A$0.90/day @ 500 commerce analyses × 60% paid rate × A$0.003/call | APIVoid Growth tier |
...
**Cap derivation:** A$15 = 5× projected Stage-1 worst case, brake fires ~once/month under abnormal volume, leaves headroom for incident analysis. Documented in `docs/ops/shop-signal-config.md` in PR 2.
...
| `SHOP_GUARD_CAP_USD`                | cost brake      | `15`    | Stage 1 / PR 2 | Daily cap on `feature='shop-signal'` cost telemetry              |
...
**Emergency kill-switch without deploy:** `INSERT INTO feature_brakes (feature, paused_until) VALUES ('shop-signal', now() + interval '7 days')` flips the paid feed off; the Stage-0 free-only path still runs.
...
1. **APIVoid trial sign-up** — confirm A$0.003/call rate, confirm AU IP egress isn't extra. Owner: brendan. Pre-PR-2.
2. **Stage-0 measurement queries** — which `analysis_events` columns / Plausible events do we need? Draft the SQL before PR 1 ships so we don't lose 30 days waiting on tooling.
3. **In-app-browser fingerprint list** — which IG/TikTok/FB User-Agent substrings? Compile from existing UA-parsing code or stand up a one-line lookup. Pre-PR-1.
4. **Scam Report linkage decision** — accept the 4-vs-3 mismatch as a known limitation (matches Charity Check), or widen `scam_reports.verdict` constraint in a separate cross-table migration first? Default: accept, document in CONTEXT.md.
