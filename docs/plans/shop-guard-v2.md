...
> **Visual companions:**
>
> - **Planning diagram** — the Mermaid block above plus an editable Excalidraw mirror at [`assets/shop-signal-architecture.excalidraw`](./assets/shop-signal-architecture.excalidraw) (PNG: [`assets/shop-signal-architecture.png`](./assets/shop-signal-architecture.png)). The Mermaid block remains the source of truth — regenerate the Excalidraw by re-running `build_shop_signal_diagram.py` if the Mermaid changes.
> - **Current-state diagram** (added 2026-05-20, post-PR #328) — [`assets/shop-signal-current-state.excalidraw`](./assets/shop-signal-current-state.excalidraw) (PNG: [`assets/shop-signal-current-state.png`](./assets/shop-signal-current-state.png)), generated by `build_shop_signal_current_state.py`. Adds annotations the planning diagram doesn't carry: the dual call-site lockstep between `/api/analyze` and `runAnalysisCore`, the Plausible side-channel, the `scam_reports.analysis_result` JSONB persistence path, and the **Voyage embedding gap** flagged as a red callout (`scam-report-embed.ts` does NOT yet read `shopSignal.commerceFlags` — backlog candidate post-Stage-1 measurement, see `BACKLOG.md` → Voyage AI / Embeddings).
