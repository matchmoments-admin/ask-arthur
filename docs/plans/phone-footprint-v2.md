...
| S5                | 9–10  | Extension right-click → footprint web app, Vonage CAMARA-landed Inngest backfill (pager + per-monitor), PWA shortcuts. SIM Swap Heartbeat push deferred to S6 (depends on Vonage CAMARA being live).                                                                                                                                                                                                                       | ✅     |
| **S6 (re-scope)** | 11–12 | **International launch foundations**: caller-country detection (Vercel geo), regional coverage block in lookup response, **carrier-drift pillar-4 fallback** (Twilio Lookup deltas — ships AU SIM-swap-proxy without waiting for Vonage AU), international marketing copy with honest per-country claims. Original "Vonage CAMARA AU go-live" plan deferred — Vonage Network Registry doesn't include AU as of April 2026. | ✅     |
