...
- **v4 (#1082 / #1085, 2026-09-04)** — length + scam-context gate on the confusable rule; the 1-edit neighbourhood of 5-char tokens gated (brand must survive contiguously, or a context token sits outside the primary label). Cut ~17% FP from every month; June–August re-triaged. Cost: 9 confirmed threats missed.
- **v5 (#1150 / #1084, 2026-09-27)** — the 5-char neighbourhood re-opened on two paths, floored by the generated neighbour-word denylist: homoglyph substitution (every brand) and `openShortNeighbourhood` brands (Apple, Bonds). 8/9 recovered, 0 v4 matches lost, 0 dictionary words re-admitted, ~10 alerts/month added (the denylist carries a foreign/brandable supplement — appli, bonde, bondo, bondy, bondu). The store stamps the matcher by PERIOD (`MATCHER_VERSION_CUTOVERS`, v5 from 2026-10-01), so re-publishing a v4 month keeps it v4. **Known miss: woles.net** — Coles was measured and deliberately not opted in (11 → 20 in 90 days for one threat shaped like koles/noles; accuracy before brand contact). Ships with the monthly store's `targeting_events` (bulk registrations counted once). Measurement + re-triage plan: `docs/ops/clone-watch-config.md` §7b.
