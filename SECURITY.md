...
| Prompt injection via invisible Unicode    | High   | `sanitizeUnicode()` strips zero-width/bidi/tag-block chars + NFKC; `detectInjectionAttempt()` also REPORTS tag-block, bidi-override, and zero-width-run characters as a manipulation signal (SUSPICIOUS floor), and folds text before phrase matching so fullwidth/zero-width-split phrasing can't evade the pre-filter                                                                                                                                                                                                                                                                                      |
...
| Threat intel data exfiltration            | Medium | Views use `security_invoker = true`; service-role access only; no public API exposure. **Corrected 2026-07-29:** this row overstated the position for two months — `scam_urls` (413,190), `scam_ips` (723,478), `scam_crypto_wallets`, `scam_clusters` and `cluster_members` still carried `USING (true)` anon SELECT policies and were readable with the public anon key, bypassing `guardV1` quota/billing. Closed in **v259**. See the dedicated row below.                                                                                                                                               |
...
| Anon bulk-export of scam-entity contacts  | High   | With the public anon key, `/rest/v1/scam_entities?select=*` returned all rows incl. raw scammer phone/email in `normalized_value` (found live 2026-05-30, 93 rows). **Remediated in v172 (#566), applied to prod 2026-05-30** — dropped anon Public-read RLS on `scam_entities` / `scam_reports` / `report_entity_links` (post-apply verified: anon SELECT returns 0 rows); readers are service_role / the rate-limited `/api/scam-contacts/lookup`                                                                                                                                                          |
| Anon bulk-export of the threat corpus     | High   | v172 fixed three tables; five more kept `USING (true)` anon SELECT. Measured live 2026-07-29 with the bundle's own anon key: `scam_urls` 413,190 rows, `scam_ips` 723,478, plus `scam_crypto_wallets` / `scam_clusters` / `cluster_members`. PostgREST sits outside the Next.js middleware, so the scrape was unrated and unbilled, and it let an operator diff the corpus daily to see which of their domains were burned. **Remediated in v259** — verified no browser-side reader, no SECURITY INVOKER function reader, and no write policy to break (scrapers use a direct `SUPABASE_DB_URL` connection) |
| Unauthenticated service-role API routes   | High   | `/api/dashboard/threat-feed` carried a comment claiming "dashboard routes are behind auth in layout" and performed no check — `middleware.ts` protects `/app` and `/admin` _pages_, but `/api/*` gets rate limiting only. It served raw `scam_entities.normalized_value` over the service-role client to anyone (verified live 2026-07-29, HTTP 200, no credentials), routing around the v172 RLS fix at the HTTP layer. **Fixed 2026-07-29**: real session check, and `normalized_value` removed from the projection so an auth regression cannot re-leak identifiers                                       |
...
| S2  | Unicode prompt injection         | **DONE** | `sanitizeUnicode()` in `scam-engine/claude.ts` — strips zero-width/bidi/tag-block classes, NFKC normalize; since 2026-08-21 `detectInjectionAttempt()` also flags these characters (UNICODE_OBFUSCATION_PATTERNS) instead of only stripping them |
