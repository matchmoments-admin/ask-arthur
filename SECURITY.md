...
| Anon bulk-export of scam-entity contacts  | High   | With the public anon key, `/rest/v1/scam_entities?select=*` returned all rows incl. raw scammer phone/email in `normalized_value` (verified live 2026-05-30, 93 rows). **Remediated in v172 (#566), pending prod apply** — drops anon Public-read RLS on `scam_entities` / `scam_reports` / `report_entity_links`; readers move to service_role / the rate-limited `/api/scam-contacts/lookup` |
