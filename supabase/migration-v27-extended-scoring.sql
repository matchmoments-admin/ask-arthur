-- Migration v27: Extend risk scoring RPC with external intelligence signals.
-- Adds scoring for:
--   enrichment_data -> 'abuseIPDB' -> { abuseConfidenceScore, totalReports }
--   enrichment_data -> 'hibp' -> { isBreached, breachCount }
--   enrichment_data -> 'ctLookup' -> { certificateCount, newestCertDate }
--   enrichment_data -> 'twilioLookup' -> { isVoip, countryCode }
--   enrichment_data -> 'urlscan' -> { malicious, score }
--
-- Enrichment points cap raised from 25 → 40 to accommodate new signals.

-- =============================================================================
-- Replace compute_entity_risk_score with extended signal checks
-- =============================================================================
...
BEGIN
  ...
  -- 3. Enrichment signals (0-40 pts) — raised from 25 for new external intel
  ...
  IF v_entity.enrichment_status = 'completed'
    ...
    IF v_entity.entity_type = 'phone' THEN
      ...
      -- Twilio VoIP confirmed: +12 (replaces local-intel +10 if both present)
      IF v_entity.enrichment_data->'twilioLookup' IS NOT NULL THEN
        IF (v_entity.enrichment_data->'twilioLookup'->>'isVoip')::BOOLEAN = TRUE THEN
          -- Remove the +10 from local intel VoIP if already added, add +12 instead
          IF v_enrichment_signals @> '"voip_phone"'::JSONB THEN
            v_enrichment_points := v_enrichment_points - 10 + 12;
          ELSE
            v_enrichment_points := v_enrichment_points + 12;
          END IF;
          v_enrichment_signals := v_enrichment_signals || '"twilio_voip_confirmed"'::JSONB;
        END IF;
        -- Twilio: non-AU origin: +8
        IF (v_entity.enrichment_data->'twilioLookup'->>'countryCode') IS NOT NULL
           AND (v_entity.enrichment_data->'twilioLookup'->>'countryCode') != 'AU' THEN
          v_enrichment_points := v_enrichment_points + 8;
          v_enrichment_signals := v_enrichment_signals || '"twilio_non_au_origin"'::JSONB;
        END IF;
      END IF;
    ...
    ELSIF v_entity.entity_type = 'email' THEN
      ...
      END IF;
      -- HIBP: email in data breaches: +5
      IF v_entity.enrichment_data->'hibp' IS NOT NULL
         AND (v_entity.enrichment_data->'hibp'->>'isBreached')::BOOLEAN = TRUE THEN
        -- 5+ breaches: +8 (replaces +5)
        IF (v_entity.enrichment_data->'hibp'->>'breachCount')::INT >= 5 THEN
          v_enrichment_points := v_enrichment_points + 8;
          v_enrichment_signals := v_enrichment_signals || '"hibp_many_breaches"'::JSONB;
        ELSE
          v_enrichment_points := v_enrichment_points + 5;
          v_enrichment_signals := v_enrichment_signals || '"hibp_breached"'::JSONB;
        END IF;
      END IF;
    ...
    ELSIF v_entity.entity_type = 'domain' THEN
      ...
      END IF;
      -- CT Lookup: cert < 30 days old: +5
      IF v_entity.enrichment_data->'ctLookup' IS NOT NULL THEN
        IF (v_entity.enrichment_data->'ctLookup'->>'newestCertDate') IS NOT NULL THEN
          IF (NOW() - (v_entity.enrichment_data->'ctLookup'->>'newestCertDate')::TIMESTAMPTZ) < INTERVAL '30 days' THEN
            v_enrichment_points := v_enrichment_points + 5;
            v_enrichment_signals := v_enrichment_signals || '"recent_cert"'::JSONB;
          END IF;
        END IF;
        -- No CT certificates at all: +8
        IF (v_entity.enrichment_data->'ctLookup'->>'certificateCount')::INT = 0 THEN
          v_enrichment_points := v_enrichment_points + 8;
          v_enrichment_signals := v_enrichment_signals || '"no_ct_certificates"'::JSONB;
        END IF;
      END IF;
    ...
    ELSIF v_entity.entity_type = 'url' THEN
      ...
      END IF;
      -- URLScan: malicious verdict: +15
      IF v_entity.enrichment_data->'urlscan' IS NOT NULL THEN
        IF (v_entity.enrichment_data->'urlscan'->>'malicious')::BOOLEAN = TRUE THEN
          v_enrichment_points := v_enrichment_points + 15;
          v_enrichment_signals := v_enrichment_signals || '"urlscan_malicious"'::JSONB;
        -- URLScan: score >= 50: +10
        ELSIF (v_entity.enrichment_data->'urlscan'->>'score')::INT >= 50 THEN
          v_enrichment_points := v_enrichment_points + 10;
          v_enrichment_signals := v_enrichment_signals || '"urlscan_suspicious"'::JSONB;
        END IF;
      END IF;
    ...
    ELSIF v_entity.entity_type = 'ip' THEN
      ...
      END IF;
      -- AbuseIPDB: confidence >= 80: +15 (higher threshold replaces lower)
      IF v_entity.enrichment_data->'abuseIPDB' IS NOT NULL THEN
        IF (v_entity.enrichment_data->'abuseIPDB'->>'abuseConfidenceScore')::INT >= 80 THEN
          v_enrichment_points := v_enrichment_points + 15;
          v_enrichment_signals := v_enrichment_signals || '"abuseipdb_high_confidence"'::JSONB;
        ELSIF (v_entity.enrichment_data->'abuseIPDB'->>'abuseConfidenceScore')::INT >= 50 THEN
          v_enrichment_points := v_enrichment_points + 10;
          v_enrichment_signals := v_enrichment_signals || '"abuseipdb_moderate_confidence"'::JSONB;
        END IF;
      END IF;
    END IF;
  END IF;

  v_enrichment_points := LEAST(40, v_enrichment_points);
