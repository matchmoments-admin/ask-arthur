...
- **Verdict merge logic** — `mergeVerdict` lives in `@askarthur/core-analysis`; `runAnalysisCore` lives in this package's `analyze-core.ts`. Don't duplicate URL-escalation or injection-floor rules here.
...
Package tests live under `src/__tests__/` and `src/inngest/__tests__/`. Typecheck: `pnpm --filter @askarthur/scam-engine typecheck`; `pnpm --filter @askarthur/web typecheck` also surfaces contract breakage since web consumes most exports.
