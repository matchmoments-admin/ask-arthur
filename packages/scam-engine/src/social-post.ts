...
export function generateDraftPosts(input: DraftPostInput): DraftPosts {
  ...
  const summaryOneLiner =
    summaryClean.length > 80 ? summaryClean.slice(0, 77) + "..." : summaryClean;
  ...
  if (withSummary.length <= 280 && summaryOneLiner) {
    ...
    short =
      shortBase.length <= 280 ? shortBase : shortBase.slice(0, 277) + "...";
  }
  ...
  if (input.scammerPhones?.length) {
    scammerInfo.push(
      `Scammer phone numbers detected: ${input.scammerPhones.slice(0, 3).join(", ")}`,
    );
  }
  if (input.scammerUrls?.length) {
    scammerInfo.push(
      `Suspicious URLs: ${input.scammerUrls.slice(0, 3).join(", ")}`,
    );
  }
...
export function generateWeeklySummary(input: WeeklySummaryInput): DraftPosts {
  ...
  const allBrandLines = input.brandCounts
    .slice(0, 8)
    .map(
      (b, i) =>
        `${i + 1}. ${b.brand} — ${b.count} scam${b.count !== 1 ? "s" : ""} detected`,
    );
  ...
  const long = [
    ...
    input.newCampaigns > 0
      ? `🆕 ${input.newCampaigns} new scam campaign${input.newCampaigns !== 1 ? "s" : ""} identified`
      : "",
    "",
