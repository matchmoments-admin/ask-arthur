...
// typed failure. It only throws on programmer error (a body form without a cap).
...
export async function safeFetch(
  ...
  try {
    for (let redirects = 0; ; redirects++) {
      ...
      if (REDIRECT_STATUSES.has(res.status) && redirect !== "manual") {
        await res.body?.cancel().catch(() => undefined);
        if (redirect === "error") return fail("redirects", "redirect-refused");
        const location = res.headers.get("location");
        if (!location) return fail("redirects", "no-location");
        let next: string;
        try {
      ...
      if (!read.ok) {
        return read.reason === "too_large"
          ...
          : fail("no_body", "empty-body");
      }
      if (as === "bytes") return { ok: true, body: read.bytes, truncated: read.truncated, ...meta };
      const text = new TextDecoder("utf-8", { fatal: false }).decode(read.bytes);
      if (as === "text") return { ok: true, body: text, truncated: read.truncated, ...meta };
      try {
        return { ok: true, body: JSON.parse(text), truncated: read.truncated, ...meta };
      } catch {
