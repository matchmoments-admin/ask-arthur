...
| Arthur's Watch — competitor-newsletter intel     | [docs/plans/arthurs-watch-newsletter.md](./docs/plans/arthurs-watch-newsletter.md) + [docs/plans/arthurs-watch-phase1-ingest-runbook.md](./docs/plans/arthurs-watch-phase1-ingest-runbook.md) + [docs/adr/0021-competitor-intel-source-class.md](./docs/adr/0021-competitor-intel-source-class.md) — competitor scam-newsletters ingested via the inbound-email pipeline under a new `competitor_intel` **source class** (ingest-but-never-publish, ADR-0021): `category='competitor_intel'` + `COMPETITOR_INTEL_SOURCES` gate exception + promote-guard. Phase 1 ingest SHIPPED (9 competitor sources + `wa_scamnet` regulator, migrations v209–v214); a Phase 2 extraction sub-step SHIPPED — `competitor_intel_observations` (v212) + the 6h `competitor-intel-extract` cron, gated `FF_COMPETITOR_INTEL_EXTRACT` (default OFF, ON in prod, code pending merge). Cost cap shares `feature_brakes.reddit_intel` / `REDDIT_INTEL_CAP_USD`; spend tagged `cost_telemetry WHERE feature='competitor-intel-extract'`. |
...
| Clone-watch MVP — plan + ops                     | [docs/plans/clone-watch-mvp.md](./docs/plans/clone-watch-mvp.md) — Layer 0 of ADR-0016 (pre-Stage-1 NRD lexical sweep against ~50 AU brands). S0E.1 shipped 2026-05-24 (v140 schema + matcher), S0E.2 shipped (Inngest fn + v141 RPC, gated `FF_SHOPFRONT_CLONE_WATCH` default OFF), S0E.3 shipped (public `/clone-watch` page, `noindex` for first 7 days until #371 v1 copy returns). Cost A\$0/mo — whoisds free tier. Cost-telemetry label `shopfront_clone_watch` (snake-case, distinct from Phase A's `shopfront_clone_scan`), v2 matcher (PR #408) scam-context-token gate shipped 2026-05-24 — Day 1 evidence: 5 hits, 20% FP; v3 'au' token word-boundary fix tracked in #409. Ops config: [docs/ops/clone-watch-config.md](./docs/ops/clone-watch-config.md).                                                                                                                                                                                                                                             |
...
- **Build each feature to complement the whole platform, and instrument it transparently.** Before adding a parallel module, check whether an existing routing brain / RPC / view / event store already does the job and **extend it** (single source of truth) — see [Onward reporting & jurisdiction routing](#onward-reporting--jurisdiction-routing) for the canonical example. Every feature must be observable: Axiom logs (rare high-value events use always-ship `.warn`, bypassing the 10% INFO sampling; high-volume paths stay on sampled `withAxiomLogging`; gate on `FF_AXIOM_ENABLED`), `logCost()` tagged `feature`+`provider` for any paid API — free-tier still logs `units` at `estimatedCostUsd: 0` so volume/ceiling stays visible — and a `feature_brakes` kill-switch wherever spend is involved. Prefer additive, pure, zero-latency integration on hot paths; check the IO/index budget before adding indexes to hot tables
...
### Onward reporting & jurisdiction routing

The "report this scam" flow already has a single **routing brain** — extend it, do not build a parallel resolver:

- **Routing brain (the logged, multi-destination onward flow):** the `get_onward_destinations` RPC (`supabase/migration-v119-onward-reporting.sql`) decides which destinations to emit from scam attributes (financial loss / PII compromise / channel / impersonated brand). For the click-triggered flow (`OnwardReportPicker` → `onward_report_log`), new routing dimensions (e.g. jurisdiction) belong here as an RPC arg + a new migration (v119 is immutable). **The one exception** is the synchronous, non-logged "best next step" nudge (`NextStepsCard` / `apps/web/lib/nextStep.ts`): to stay zero-latency on the analyze response it runs a pure, client/server-shared resolver over the SAME destination data — it presents the single _best_ link, it does not log or fan out. It must not fork the destination data (that stays a single source of truth in `destinations.ts`), only the presentation.
- **Audit substrate:** every attempt is one `onward_report_log` row (dedup unique on `(scam_report_id, destination, destination_key)`); `POST /api/report/onward` queues + fans out to per-destination Inngest workers (`apps/web/app/api/inngest/functions/onward-*.ts`). Adding a destination = an enum value + a worker + a `FIXED_DESTINATION_KEYS` entry (`apps/web/app/api/report/onward/route.ts`).
...
- **Latent gap:** `ResultCard` does not pass `hasFinancialLoss`/`hasPiiCompromise` to `OnwardReportPicker`, so ReportCyber + IDCARE never surface to consumers today — fix when wiring any deeper reporting surface.
