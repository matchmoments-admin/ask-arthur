...
| News Intel feeds — ops                      | AU regulator narrative scrapers (Scamwatch HTML, ACSC RSS, ASIC JSON) shipped 2026-05-06 (PR #137 + fixes #138/#139, migration v97). Scrapers in `pipeline/scrapers/{scamwatch,acsc,asic_investor}_alerts.py` write to `feed_items` with `source IN ('scamwatch_alert','acsc','asic_investor')`. Voyage embedding via `feed-items-embed` Inngest cron (`*/30 * * * *`). Weekly digest folds in via `regulatorAlerts` section in `WeeklyIntelDigest.tsx`. **Retention** (migration v98): narrative `feed_items` >365d → `feed_items_archive`; `feed_ingestion_log` pruned 90d; `feed_http_cache` pruned 30d. All housekeeping runs nightly at 02:30 UTC via `feed-retention` Inngest function. **Known issue**: `cyber.gov.au` RSS occasionally times out from GitHub Actions IPs (suspected UA filtering by Cloudflare WAF) — `common/http_cache.py` falls back to a Mozilla UA on retry; persistent failures are logged cleanly to `feed_ingestion_log` so the 3h cron self-heals on next reachable window.                                                                                                                                                                                                                                                                                                                                                                                                 |
| Vulnerability Intelligence — research + ops | [docs/vulnerability-atlas-2026.md](./docs/vulnerability-atlas-2026.md) (2026 research) + [docs/phase-14-config.md](./docs/phase-14-config.md) (ops config). **Sprints 0–2 shipped 2026-04-21/22** (PRs #5, #6, #8; migrations v63/v64) — VIDB schema (`vulnerabilities`, `vulnerability_exposure_checks`, `vulnerability_detections`, `vulnerability_ingestion_log`), 4 scrapers (`cisa_kev.py`, `nvd_recent.py`, `osv_feed.py`, `github_advisory.py` in `pipeline/scrapers/vulnerabilities/`), hourly `enrich-vulnerability.ts` AU-context cron with `ffVulnAuEnrichment` flag + `feature_brakes.vuln_au_enrichment` brake, 12 hardcoded MCP CVEs in `packages/mcp-audit/src/cve-rulepack.ts`, `/admin/vulnerabilities` dashboard. Cron cadence trimmed 2026-05-02 (PR #76). **Sprints 3–5 planned** (extension hardening, B2B `/api/v1/vulnerabilities/*`, consumer URL-security-report depth). **Sprint 6 — Enrichment depth & scoring scoped 2026-05-11** (plan-only): AUSCERT scraper, MSRC monthly CVRF, daily EPSS refresh, Arthur Risk Score + ADR 0007, VulnCheck Community SSVC, mcp-audit live DB-driven feed + OWASP-LLM/MITRE ATLAS taxonomy seed, ASD Cyber Security Network Partner application. **Out-of-scope-decided** (per 2026-05-11 review): SBOM ingest, customer software inventory tables, AI-assisted auto-PR remediation, internal bounty agent, $A49–$A299 B2B SMB pricing tiers. |
