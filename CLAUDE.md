...
| SIM Swap — consumer beta       | Telstra-direct CAMARA SIM-swap check at [/sim-swap-check](./apps/web/app/sim-swap-check/) (private beta, invite-gated). Provider in `packages/scam-engine/src/phone-footprint/providers/telstra.ts`; endpoint `apps/web/app/api/sim-swap/check/route.ts`. Pricing: 1 free check / user / month + $0.99 5-pack + $4.99 KYC-gated recovery check. Flags: `FF_TELSTRA_SIM_SWAP_ENABLED` (server) + `NEXT_PUBLIC_FF_SIM_SWAP_ON_DEMAND` (client). Schema in migrations v123/v125/v126. Rollout checklist appended to [docs/ops/phone-footprint-config.md](./docs/ops/phone-footprint-config.md). PRs #219–#223 stacked; consumer beta engineering complete, awaiting Telstra trial creds + Stripe product setup + flag flip.        |
...
**Cost brakes:** `DAILY_COST_THRESHOLD_USD` (default `2`), `VULN_AU_ENRICHMENT_CAP_USD` (`5`), `REDDIT_INTEL_CAP_USD` (`10`), `PHONE_FOOTPRINT_CAP_USD` (`5`), `CHARITY_CHECK_CAP_USD` (`5`). Use bare numbers — `parseFloat("$10")` is `NaN` and silently disables the brake.
