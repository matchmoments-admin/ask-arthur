...
- **"shadow lane"** — a classifier (or any scorer) run beside the live one on the _identical_ input, with its output persisted and **read by nothing in the product path**. It exists only to be measured against stored outcomes before any gate is allowed to read it, and it carries its own delete rule. First instance: the Jev shadow lane on the clone-watch pre-classifier (v311, `clone_watch_jev_classifications`, `clone_watch_jev_calibration()`). Distinct from a **canary** (a live path serving a fraction of real traffic) and from a **dry-run** (the live path with its side effects suppressed).
