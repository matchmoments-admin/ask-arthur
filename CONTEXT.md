...
**Structural Finding** (Document Check Module):
One named, explainable editing-trace signal read directly from an uploaded document's bytes — `multiple_revisions`, `producer_design_tool`, `dates_differ`, etc. (`DOCUMENT_FINDING_SIGNALS` in `packages/types/src/document-check.ts`). The Document Check Module (`packages/scam-engine/src/document-check/`, seam: `inspectDocument(buffer)`) is the document sibling of the Image Check Module and inherits its epistemics wholesale: findings are facts about the FILE ("records 2 incremental updates"), never a verdict about the DOCUMENT — there is no composite score and no FAKE/GENUINE output, per ADR-0015/0024. The asymmetry rule carries over: zero findings means "no editing traces found", never "genuine" — a fraudulent document generated from scratch has no revision history at all. Two layers: the **structural** layer is jurisdiction-agnostic byte forensics; the **content** layer is per-jurisdiction validator packs (AU: ABN/BSB/arithmetic — `content: null` = "not assessed", the ABN-Status `unverified` discipline). Canonical copy in `DOCUMENT_CHECK_COPY` (`packages/types/src/document-check-copy.ts`), guarded by `apps/web/__tests__/documentCheckCopy.test.ts`.
_Avoid_: "document verification" (implies a verdict we never render); "forensic score" (no scores — named signals only); "tamper detection" as a promise (we detect _traces_; their absence proves nothing).
