...
export async function prepareNewsletter(sb: Client, now = new Date()) {
  ...
  const [takes, regulators, clones] = await Promise.all([
    sb.from("reddit_post_intel").select("feed_item_id,narrative_summary,take_status,take_tells,take_where,take_au_line,confidence,country_hints,intent_label,feed_items!inner(title,source_created_at,published,source)")
      .eq("take_status", "ready").gte("confidence", 0.7).eq("is_scam_report", true)
  ...
  for (const row of rows) {
    ...
    candidates.push({
      id: `take:${row.feed_item_id}`, title: feed.title.slice(0, 140), summary: row.narrative_summary.slice(0, 650),
      ...
      sourceLabel: "Arthur’s Take — Reddit report", sourceUrl: `https://askarthur.au/scam-feed/${takeSlug(row.feed_item_id, feed.title)}`,
