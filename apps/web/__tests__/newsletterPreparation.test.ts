...
function fixture(failedTable?: string) {
  ...
  const take = (id: number, date: string, published = true) => ({ feed_item_id: id, confidence: 0.9, take_status: "ready", is_scam_report: true, take_tells: ["Pressure", "Secrecy"], narrative_summary: "A person reported a suspicious request.", take_au_line: "Check independently.", country_hints: ["AU"], processed_at: "2026-09-06T00:00:00Z", feed_items: feed(date, published) });
  const tables: Record<string, Row[]> = {
    newsletter_issues: [], reddit_post_intel: [take(1, "2026-09-03T00:00:00Z"), take(2, "2025-01-01T00:00:00Z"), take(3, "2026-09-03T00:00:00Z", false), take(4, "2026-09-07T00:00:00Z")],
  ...
  const sb = { from(table: string) {
    ...
    const q = {
      ...
      upsert: (row: Row) => { if (!tables[table].length) tables[table].push({ ...row, id: "issue" }); return q; },
...
it("excludes reprocessed old posts, unpublished sources and the end boundary", async () => {
  const { sb } = fixture();
